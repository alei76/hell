...
import ps.hell.ml.statistics.distibution.DiscreteDistribution;
...
public class QQPlot extends Plot {
    ...
    public static PlotCanvas plot(double[] x) {
        double[] lowerBound = {MathBase.min(x), GaussianDistribution.getInstance().quantile(1 / (x.length + 1.0))};
        double[] upperBound = {MathBase.max(x), GaussianDistribution.getInstance().quantile(x.length / (x.length + 1.0))};
    ...
    public static PlotCanvas plot(double[] x, Distribution d) {
        double[] lowerBound = {MathBase.min(x), d.quantile(1 / (x.length + 1.0))};
        double[] upperBound = {MathBase.max(x), d.quantile(x.length / (x.length + 1.0))};
        PlotCanvas canvas = new PlotCanvas(lowerBound, upperBound);
    ...
    public static PlotCanvas plot(double[] x, double[] y) {
        double[] lowerBound = {MathBase.min(x), MathBase.min(y)};
        double[] upperBound = {MathBase.max(x), MathBase.max(y)};
        PlotCanvas canvas = new PlotCanvas(lowerBound, upperBound);
