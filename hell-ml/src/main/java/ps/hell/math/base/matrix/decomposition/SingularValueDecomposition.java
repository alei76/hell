...
public class SingularValueDecomposition {
	...
	public SingularValueDecomposition(Matrix Arg) {
		...
		double tiny = (double)Math.pow(2.0, -966.0);
		while (p > 0) {
			int k, kase;
			...
			if (k == p - 2) {
				...
				for (ks = p - 1; ks >= k; ks--) {
					...
					double t = (ks != p ? Math.abs(e[ks]) : 0f)
							+ (ks != k + 1 ? Math.abs(e[ks - 1]) : 0f);
