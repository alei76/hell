 ...
 * @author Haifeng Li
 */
public class SingularValueDecomposition2 {
     ...
     * the decomposition.
     */
    private SingularValueDecomposition2(double[][] U, double[][] V, double[] s) {
        this(U, V, s, true);
    }
     ...
     * the decomposition.
     */
    private SingularValueDecomposition2(double[][] U, double[][] V, double[] s, boolean full) {
        this.U = U;
        this.V = V;
        this.s = s;
        this.full = full;
        ...
        m = U.length;
        n = V.length;
        tol = 0.5 * Math.sqrt(U.length + V.length + 1.0) * s[0] * MathBase.EPSILON;
    }
     ...
     */
    public static SingularValueDecomposition2 decompose(IMatrix A, int k) {
     ...
     */
    public static SingularValueDecomposition2 decompose(IMatrix A, int k, double kappa) {
        ATA B = new ATA(A);
        EigenValueDecomposition2 eigen = EigenValueDecomposition2.decompose(B, k, kappa);
        ...
        if (A.nrows() >= A.ncols()) {
            ...
            return new SingularValueDecomposition2(U, V, s, false);
     ...
     */
    public static SingularValueDecomposition2 decompose(double[][] A) {
        int m = A.length;
        int n = A[0].length;
        ...
        for (k = n - 1; k >= 0; k--) {
            for (its = 0; its < 30; its++) {
                ...
                for (l = k; l >= 0; l--) {
                    nm = l - 1;
                    if (l == 0 || Math.abs(rv1[l]) <= MathBase.EPSILON * anorm) {
                        flag = false;
                        break;
                    }
                    if (Math.abs(w[nm]) <= MathBase.EPSILON * anorm) {
                        break;
                    }
                }
                ...
                if (flag) {
                    ...
                    for (i = l; i < k + 1; i++) {
                        f = s * rv1[i];
                        rv1[i] = c * rv1[i];
                        if (Math.abs(f) <= MathBase.EPSILON * anorm) {
                            break;
                        }
                        g = w[i];
                        h = Math.hypot(f, g);
        ...
        return new SingularValueDecomposition2(u, v, w);
    }
}
