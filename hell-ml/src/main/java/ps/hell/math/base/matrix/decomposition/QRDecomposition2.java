...
public class QRDecomposition2 {
     ...
     */
    public QRDecomposition2(double[][] A) {
        this(A, false);
    }
     ...
     */
    public QRDecomposition2(double[][] A, boolean overwrite) {
        // Initialize.
        int m = A.length;
        int n = A[0].length;
     ...
     */
    public CholeskyDecomposition2 toCholesky() {
        int n = QR[0].length;
        ...
        return CholeskyDecomposition2.newInstance(L);
    }
    ...
    public double[][] inverse() {
        double[][] I = MathBase.eye(QR[0].length, QR.length);
        solve(I);
        return I;
    }
    ...
    public void update(double[] u, double[] v) {
        ...
        for (int i = k - 1; i >= 0; i--) {
            ...
            if (u[i] == 0.0) {
                ...
                u[i] = Math.abs(u[i]) * Math.sqrt(1.0 + MathBase.sqr(u[i + 1] / u[i]));
            } else {
                u[i] = Math.abs(u[i + 1]) * Math.sqrt(1.0 + MathBase.sqr(u[i] / u[i + 1]));
            }
        }
