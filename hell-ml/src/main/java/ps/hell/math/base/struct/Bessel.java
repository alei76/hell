...
static public double i0(double x) {
	...
	if( x <= 8.0 ) {
		y = (x/2.0) - 2.0;
		return( Math.exp(x) * MathBase.chbevl( y, A_i0, 30 ) );
	}

	return(  Math.exp(x) * MathBase.chbevl( 32.0/x - 2.0, B_i0, 25 ) / Math.sqrt(x) );
}
/**
...
static public double i0e(double x) {
	...
	if( x <= 8.0 ) {
		y = (x/2.0) - 2.0;
		return( MathBase.chbevl( y, A_i0, 30 ) );
	}

	return(  MathBase.chbevl( 32.0/x - 2.0, B_i0, 25 ) / Math.sqrt(x) );
}
/**
...
static public double i1(double x) {
	...
	if( z <= 8.0 )
		{
		y = (z/2.0) - 2.0;
		z = MathBase.chbevl( y, A_i1, 29 ) * z * Math.exp(z);
		}
	else
		{
		z = Math.exp(z) * MathBase.chbevl( 32.0/z - 2.0, B_i1, 25 ) / Math.sqrt(z);
		}
	if( x < 0.0 )
		z = -z;
	return( z );
...
if( z <= 8.0 )
	{
	y = (z/2.0) - 2.0;
	z = MathBase.chbevl( y, A_i1, 29 ) * z;
	}
else
	{
	z = MathBase.chbevl( 32.0/z - 2.0, B_i1, 25 ) / Math.sqrt(z);
	}
if( x < 0.0 )
	z = -z;
return( z );
...
static public double k0(double x) {
	...
	if( x <= 2.0 ) {
		y = x * x - 2.0;
		y = MathBase.chbevl( y, A_k0, 10 ) - Math.log( 0.5 * x ) * i0(x);
		return( y );
	}
	
	z = 8.0/x - 2.0;
	y = Math.exp(-x) * MathBase.chbevl( z, B_k0, 25 ) / Math.sqrt(x);
	return(y);
}
/**
 * Returns the exponentially scaled modified Bessel function
...
static public double k0e(double x) {
	...
	y = MathBase.chbevl( 8.0/x - 2.0, B_k0, 25 ) / Math.sqrt(x);
	return(y);
}
/**
 * Returns the modified Bessel function of the third kind
...
static public double k1(double x) {
	...
	if( x <= 2.0 ) {
		y = x * x - 2.0;
		y =  Math.log(z) * i1(x)  +  MathBase.chbevl( y, A_k1, 11 ) / x;
		return( y );
	}

	return(  Math.exp(-x) * MathBase.chbevl( 8.0/x - 2.0, B_k1, 25 ) / Math.sqrt(x) );
}
/**
 * Returns the exponentially scaled modified Bessel function
...
static public double k1e(double x) {
	...
	if( x <= 2.0 ) {
		y = x * x - 2.0;
		y =  Math.log( 0.5 * x ) * i1(x)  +  MathBase.chbevl( y, A_k1, 11 ) / x;
		return( y * Math.exp(x) );
	}

	return(  MathBase.chbevl( 8.0/x - 2.0, B_k1, 25 ) / Math.sqrt(x) );
}
/**
 * Returns the modified Bessel function of the third kind
...
*/
	...
	if( x <= 9.55 ) {
		...
		while( Math.abs(t/s) >MathBase.MACHEP );
	...
	if( x > MathBase.MAXLOG )  throw new ArithmeticException("Underflow");
	k = n;
	pn = 4.0 * k * k;
	pk = 1.0;
	z0 = 8.0 * x;
