...
import ps.hell.math.base.struct.IntArrayList;
import ps.hell.math.base.MathBase;
import ps.hell.ml.statistics.distibution.GaussianDistribution;
import ps.hell.base.sort.HeapSelect;
...
public class LSH <E> implements NearestNeighborSearch<double[], E>, KNNSearch<double[], E>, RNNSearch<double[], E> {
    ...
    class Hash {
         ...
         * @param x the vector to be hashed. x是已经获取的hash值
         ...
         * @param r universal hashing random integers. 随机化值
        ...
        BucketEntry get(double[] x) {
            ...
            //获取对应的hashtable 的index
            HashEntry he = table[i];
            if (he == null) {
                return null;
            }
            //存储了所有分配的table 的数据包括 桶号和值
            //table的结构为根据桶号再次划分
...
* w确定桶间隔
    ...
    public LSH(int d, int L, int k, double w, int H) {
        ...
        r2 = new int[k];
        for (int i = 0; i < k; i++) {
            r1[i] = MathBase.randomInt(MAX_HASH_RND);
            r2[i] = MathBase.randomInt(MAX_HASH_RND);
        }
    ...
    public Neighbor<double[], E> nearest(double[] q) {
        ...
        for (int index : candidates) {
            ...
            double distance = MathBase.distance(q, key);
