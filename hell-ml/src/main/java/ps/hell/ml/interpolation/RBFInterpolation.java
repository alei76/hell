...
public class RBFInterpolation {
    ...
    public RBFInterpolation(double[][] x, double[] y, RadialBasisFunction rbf, boolean normalized) {
        ...
        if (rbf instanceof GaussianRadialBasis) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                double sum = 0.0;
                for (int j = 0; j <= i; j++) {
                    G[i][j] = rbf.f(MathBase.distance(x[i], x[j]));
                    sum += 2 * G[i][j];
                }
            ...
            CholeskyDecomposition2 cholesky = new CholeskyDecomposition2(G, true);
            cholesky.solve(rhs);
            w = rhs;
        } else {
            ...
            LUDecomposition2 lu = new LUDecomposition2(G, true);
            lu.solve(rhs);
            w = rhs;
        }
    }
    ...
    public double interpolate(double... x) {
        ...
        for (int i = 0; i < this.x.length; i++) {
            double f = rbf.f(MathBase.distance(x, this.x[i]));
            sumw += w[i] * f;
            sum += f;
        }
