...
/**
 * 正规化的判别分析。RDA是一种妥协LDA和QDA之间,
*一个可以收缩的单独的协方差QDA常见
*方差如LDA。这种方法非常类似的岭回归的味道。
*每个类的正规化的协方差矩阵
 ...
 * @author Haifeng Li
 */
public class RDA implements Classifier<double[]> {
    ...
    public RDA(double[][] x, int[] y, double[] priori, double alpha, double tol) {
        ...
        for (int i = 0; i < k; i++) {
            ...
            EigenValueDecomposition2 eigen = EigenValueDecomposition2.decompose(cov[i], true);
