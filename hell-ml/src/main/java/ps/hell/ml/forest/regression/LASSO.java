 ...
 * 至少绝对收缩和选择算子。
*套索是收缩和选择线性回归的方法。
*它最小化通常的平方误差的总和,绑定的总和
*系数的绝对值(即L <子> 1 < /订阅>正规化)。
*它有连接对振动的小波系数,前进
*级间回归,和提高方法
...
public class LASSO  implements Regression<double[]> {
    ...
    public LASSO(double[][] x, double[] y, double lambda, double tol, int maxIter) {
        ...
        for (; ntiter <= maxIter; ntiter++) {
            ...
            MathBase.atx(X, nu, xnu);
            double maxXnu = MathBase.normInf(xnu);
            if (maxXnu > lambda) {
            ...
            pobj = MathBase.dot(z, z) + lambda * MathBase.norm1(w);
            dobj = Math.max(-0.25 * MathBase.dot(nu, nu) - MathBase.dot(nu, Y), dobj);
            if (ntiter % 10 == 0) {
            ...
            double normg = MathBase.norm(grad);            
            ...
            double error = MathBase.solve(pcg, grad, dxu, pcgtol, 1, pcgmaxi);
            if (error > pcgtol) {
                pitr = pcgmaxi;
            }
            ...
            double phi = MathBase.dot(z, z) + lambda * MathBase.sum(u) - sumlogneg(f)/t;
            s = 1.0;
            double gdx = MathBase.dot(grad, dxu);
            ...
            for (; lsiter < MAX_LS_ITER; lsiter++) {
                ...
                if (MathBase.max(newf) < 0.0) {
                    MathBase.ax(X, neww, newz);
                    for (int i = 0; i < n; i++) {
                        newz[i] -= Y[i];
                    }
                    
                    double newphi = MathBase.dot(newz, newz) + lambda * MathBase.sum(newu) - sumlogneg(newf) / t;
