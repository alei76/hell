...
public class PPCA implements Projection<double[]> {
    ...
    public double[] project(double[] x) {
        ...
        MathBase.ax(projection, x, y);
        y=MathBase.minus(y, pmu);
        return y;
    }
    ...
    public double[][] project(double[][] x) {
        ...
        for (int i = 0; i < x.length; i++) {
            MathBase.ax(projection, x[i], y[i]);
            y[i]=MathBase.minus(y[i], pmu);
        }
        return y;
    }
    ...
    public PPCA(double[][] data, int k) {
        ...
        EigenValueDecomposition2 eigen = EigenValueDecomposition2.decompose(cov, true);
        ...
        double[][] M = MathBase.atamm(loading);
        for (int i = 0; i < k; i++) {
            M[i][i] += noise;
        }

        LUDecomposition2 lu = new LUDecomposition2(M, true);
        double[][] Mi = lu.inverse();
        projection = MathBase.abtmm(Mi, loading);

        pmu = new double[projection.length];
        MathBase.ax(projection, mu, pmu);
    }
}
