...
/**
 * 广义Hebbian算法。GHA是一个线性前馈神经
*网络模型与应用程序主要是在无监督学习
*主成分分析。这是单层的过程——也就是说,一个
*突触权重变化只取决于输入和响应
*输出层
...
public class GHA implements Projection<double[]> {
    ...
    public double[][] project(double[][] x) {
        ...
        for (int i = 0; i < x.length; i++) {
            MathBase.ax(projection, x[i], wx[i]);
        }
        return wx;
    }
    ...
    public double learn(double[] x) {
        ...
        MathBase.ax(projection, x, y);
        MathBase.atx(projection, y, wy);
        return MathBase.squaredDistance(x, wy);
    }
}
