...
public class BpAnn {
	 ...
	 *            文件长度
	 */
	public BpAnn(int a, double alph, double chognliang11, int c, int con,
			int filelength) {
		// 初始化学习率
	 ...
	 *            dimensionality;//输入层维度
	 */
	public BpAnn(int a, double alph, double chognliang11, int con) {
		// 初始化学习率
		this.implicitStrataNum = a;// 存储隐层变量数
		this.dimensionality = con;// 存储维度
		this.alpha = alph;
...
//			dataoutput.add(n);
//		}
		 Normal n1=new Normal(2,0.3);
		 Matrix m1=n1.random(100,2,1);
		
		 Normal n2=new Normal(3,0.3);
		 Matrix m2=n2.random(100,2,2);
		
		 m1.addMatrix(m2);
		
		 Normal n3=new Normal(-3,0.3);
		 Matrix m3=n3.random(100,1,3);
		 Normal n32=new Normal(3,0.4);
		 Matrix m32=n32.random(100,1);
		 m32.addColMatrix(m3);
		 m1.addMatrix(m32);
		 System.out.println("m32:");
		 for(int i=0;i<m32.data.length;i++)
		 {
			 for(int j=0;j<3;j++)
			 {
				 System.out.print(m32.data[i][j]+"\t");
			 }
			 System.out.println();
		 }
		...
		// 调用第二个初始化
		BpAnn bp_test = new BpAnn(y_len, alp, impulse, i_len);
		// 执行将输出标准化过程
