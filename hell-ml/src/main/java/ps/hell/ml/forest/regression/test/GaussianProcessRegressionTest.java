...
public class GaussianProcessRegressionTest {
    ...
    public void testLearn() {
        ...
        MathBase.normalize(longley);

        int n = longley.length;
        LOOCV loocv = new LOOCV(n);
        double rss = 0.0;
        for (int i = 0; i < n; i++) {
            double[][] trainx = MathBase.slice(longley, loocv.train[i]);
        ...
        try {
            ...
            for (int i = 0; i < k; i++) {
                ...
                for (int l = 0; l < centers.length; l++) {
                    for (int j = 0; j < l; j++) {
                        r0 += MathBase.distance(centers[l], centers[j]);
                    }
                }
                r0 /= (2 * centers.length);
    ...
    public void test2DPlanes() {
        ...
        try {
            ...
            int[] perm = MathBase.permutate(x.length);
    ...
    public void testKin8nm() {
        ...
        parser.setResponseIndex(8);
        try {
            AttributeDataset data = parser.parse(this.getClass().getResourceAsStream("/smile/data/weka/regression/kin8nm.arff"));
            double[] y = data.toArray(new double[data.size()]);
            double[][] x = data.toArray(new double[data.size()][]);
