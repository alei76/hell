...
import ps.landerbuluse.ml.forest.classification.NaiveBayes;
import ps.landerbuluse.ml.math.MathBase;
import ps.landerbuluse.ml.statistics.distibution.Distribution;
import ps.landerbuluse.ml.statistics.distibution.GaussianMixture;
...
public class NaiveBayesTest {
    ...
    public void testPredict() {
        ...
        try {
            ...
            int error = 0;
            for (int l = 0; l < n; l++) {
                double[][] trainx = MathBase.slice(x, loocv.train[l]);
                int[] trainy = MathBase.slice(y, loocv.train[l]);

                int p = trainx[0].length;
                int k = MathBase.max(trainy) + 1;
    ...
    public void testLearnMultinomial() {
        ...
        int total = 0;
        for (int i = 0; i < k; i++) {
            double[][] trainx = MathBase.slice(x, cv.train[i]);
            ...
            double[][] testx = MathBase.slice(x, cv.test[i]);
            int[] testy = MathBase.slice(y, cv.test[i]);
