...
import ps.hell.ml.forest.model.projection.GHA;
...
public class GHATest {
    ...
    public void testLearn() {
        ...
        int k = 3;
        double[] mu = MathBase.colMean(USArrests);
        double[][] cov = MathBase.cov(USArrests);
        for (int i = 0; i < USArrests.length; i++) {
        	USArrests[i]=MathBase.minus(USArrests[i], mu);
        }
        ...
        double[][] s = MathBase.abtmm(MathBase.abmm(p, cov), p);
