...
public class OLS implements Regression<double[]> {
    ...
    public OLS(double[][] x, double[] y) {
        ...
        QRDecomposition2 qr = new QRDecomposition2(X, true);
        qr.solve(y, w1);
        ...
        MathBase.ax(x, w, yhat);

        double TSS = 0.0;
        RSS = 0.0;
        double ybar = MathBase.mean(y);
        residuals = new double[n];
        for (int i = 0; i < n; i++) {
            double r = y[i] - yhat[i] - b;
            residuals[i] = r;
            RSS += MathBase.sqr(r);
            TSS += MathBase.sqr(y[i] - ybar);
        }

        error = MathBase.sqrt(RSS / (n - p - 1));
        df = n - p - 1;
        ...
        CholeskyDecomposition2 cholesky = qr.toCholesky();
        double[][] inv = cholesky.inverse();

        coefficients = new double[p+1][4];
        for (int i = 0; i <= p; i++) {
            coefficients[i][0] = w1[i];
            double se = error * MathBase.sqrt(inv[i][i]);
            coefficients[i][1] = se;
            double t = w1[i] / se;
            coefficients[i][2] = t;
    ...
    public String toString() {
        ...
        builder.append(String.format("\t%8.4f\t%8.4f\t%8.4f\t%8.4f\t%8.4f\n", MathBase.min(r), MathBase.q1(r), MathBase.median(r), MathBase.q3(r), MathBase.max(r)));
