...
/**
 * Kernel Cache 缓存 l is the number of total data items size is the cache size
 * limit in bytes
 * 
...
abstract class Kernel extends QMatrix {
	...
	/**
	 * kernel 核函数定义 he k_function有什么区别?
	 * 
	 * @param i
	 *            i样本
	 * @param j
...
/**
 * smo 解法 // An SMO algorithm in Fan et al., JMLR 6(2005), p. 1889--1918 Solves:
 *
...
class Solver {
	...
	/**
	 * java: information about solution except alpha, because we cannot return
	 * multiple values otherwise...
	 * 
	 * @author Administrator
	 */
	...
	/**
	 * 重新构建梯度值 重新构建不活跃的远足在g中 从g_bar和c内部元素中
	 */
	...
	/**
	 * 选择 ab工作组 return 1 if already optimal, return 0 otherwise
	 * 
	 * @param working_set
	 * @return 如果为1 则结束最小ab值误差<eps值 如果为0 则继续
	 */
...
/**
 * Solver for nu-svm classification and regression additional constraint: e^T
 * \alpha = constant
 * 
 * @author Administrator
 *
...
/**
 * Q matrices for various formulations 使用缓存机制 行数据值 svc?
 * 
 * @author Administrator
 *
 */
...
/**
 * 核svc一样不过在获取 行数据时没有使用y[i]*y[j]作为调整
 * 
 * @author Administrator
 *
 */
...
public class Svm {
	...
	/**
	 * 标准svm方法 Platt's binary SVM Probablistic Output: an improvement from Lin
	 * et al.
	 * 
	 * @param l
	 * @param dec_values
	 ...
	 * @param target
	 */
	public static void svm_cross_validation(SvmProblem prob,
		...
		if ((param.svm_type == SvmParameter.C_SVC || param.svm_type == SvmParameter.NU_SVC)
				&& nr_fold < l) {
			int[] tmp_nr_class = new int[1];
			int[][] tmp_label = new int[1][];
			int[][] tmp_start = new int[1][];
	...
	public static void svm_save_model(String model_file_name, SvmModel model)
		...
		if (model.mapString != null) {
			fp.writeBytes("map");
			for (Entry<Integer, String> map : model.mapString.entrySet()) {
				fp.writeBytes(" " + map.getKey() + ":" + map.getValue());
			}
			fp.writeBytes("\n");
		}
		if(model.xClass!=null)
		{
			fp.writeBytes("xClass");
			for (String s:model.xClass) {
				fp.writeBytes(" " + s);
			}
			fp.writeBytes("\n");
			fp.writeBytes("indexMap");
			int index =0;
			for(Entry<Integer,HashMap<Integer,String>> map:model.indexMapString.entrySet())
			{
				index++;
				if(index==1)
				{
					fp.writeBytes(" "+Integer.toString(map.getKey()));
				}else{
					fp.writeBytes("@"+map.getKey());
				}
				for(Entry<Integer,String> m:map.getValue().entrySet())
				{
					fp.writeBytes(" "+m.getKey()+":"+m.getValue());
				}
			}
			fp.writeBytes("\n");
		}
			...
			while (true) {
				...
				if (cmd.startsWith("svm_type")) {
					...
					if(arg!=null)
					{
						...
						if(zn.length>0)
						{
							model.mapString=new HashMap<Integer,String>();
							model.mapStringRel=new HashMap<String,Integer>();
							for(int i=0;i<zn.length;i++)
							{
								String[] zp=zn[i].split(":");
								model.mapString.put(atoi(zp[0]),zp[1]);
								model.mapStringRel.put(zp[1],atoi(zp[0]));
							}
						}
					}
					
				}else if (cmd.startsWith("xClass")) {
					String[] zn=null;
					if(arg!=null)
					{
						zn=arg.split(" ");
				...
				}else if (cmd.startsWith("indexMap")) {
					model.indexMapString=new HashMap<Integer,HashMap<Integer,String>>();
					model.indexMapStringRel=new HashMap<Integer,HashMap<String,Integer>>();
					if(arg!=null)
					{
						String[] zn=arg.split("@");
						for(int i=0;i<zn.length;i++)
						{
							...
							int index=-1;
							String[] zn2=zn[i].split(" ");
							index=Integer.parseInt(zn2[0]);
							for(int j=1;j<zn2.length;j++)
							{
								String[] zp=zn2[j].split(":");
								m1.put(atoi(zp[0]),zp[1]);
								m2.put(zp[1],atoi(zp[0]));
							}
							model.indexMapString.put(index,m1);
							model.indexMapStringRel.put(index,m2);
						}
					}
	//				System.out.println();
	...
	public static int svm_check_probability_model(SvmModel model) {
		if (((model.param.svm_type == SvmParameter.C_SVC || model.param.svm_type == SvmParameter.NU_SVC)
				&& model.probA != null && model.probB != null)
				|| ((model.param.svm_type == SvmParameter.EPSILON_SVR || model.param.svm_type == SvmParameter.NU_SVR) && model.probA != null))
			return 1;
		else
			return 0;
	}
