 ...
 * 高斯过程回归。高斯过程是一个随机过程
*的实现由随机值与每一个点
*一系列时间(或空间),这样每一个这样的随机变量
*一个正态分布。此外,那些随机的每一个有限集合
*变量的多元正态分布
...
public class GaussianProcessRegression <T> implements Regression<T> {
    ...
    public GaussianProcessRegression(T[] x, double[] y, MercerKernel<T> kernel, double lambda) {
        ...
        CholeskyDecomposition2 cholesky = new CholeskyDecomposition2(K, true);
        cholesky.solve(y, w);
    }
    ...
    public GaussianProcessRegression(T[] x, double[] y, T[] t, MercerKernel<T> kernel, double lambda) {
        ...
        LUDecomposition2 lu = new LUDecomposition2(K, true);
        lu.solve(b, w);
    }
     ...
     * @param lambda the shrinkage/regularization parameter.
     */
    public GaussianProcessRegression(T[] x, double[] y, T[] t, MercerKernel<T> kernel, double lambda, boolean nystrom) {
        if (x.length != y.length) {
            throw new IllegalArgumentException(String.format("The sizes of X and Y don't match: %d != %d", x.length, y.length));
        }
        ...
        EigenValueDecomposition2 eigen = EigenValueDecomposition2.decompose(W);
        ...
        double[][] UD = MathBase.abmm(U, D);
        double[][] UDUt = MathBase.abtmm(UD, U);
        double[][] L = MathBase.abmm(E, UDUt);
        
        double[][] LtL = MathBase.atamm(L);
        for (int i = 0; i < m; i++) {
            LtL[i][i] += lambda;
        }
        
        double[][] invLtL = MathBase.inverse(LtL);
        double[][] K = MathBase.abtmm(MathBase.abmm(L, invLtL), L);
