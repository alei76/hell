...
/**
 * 随机投影是一种很有前途的降维技术
*学习混合高斯模型。根据Johnson-Lindenstrauss引理,
*任何n数据点可以映射到高维度
...
public class RandomProjection implements Projection<double[]> {
    ...
    public RandomProjection(int n, int p, boolean sparse) {
        ...
        if (sparse) {
            ...
            for (int i = 0; i < p; i++) {
                for (int j = 0; j < n; j++) {
                    projection[i][j] = scale * (MathBase.random(prob) - 1);
                }
            }
        } else {
            ...
            MathBase.unitize(projection[0]);
            for (int i = 1; i < p; i++) {
                for (int j = 0; j < i; j++) {
                    double t = -MathBase.dot(projection[i], projection[j]);
                    MathBase.axpy(t, projection[j], projection[i]);
                }
                MathBase.unitize(projection[i]);
            }
        }
    }
