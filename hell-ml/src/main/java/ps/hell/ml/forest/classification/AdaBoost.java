...
*演算法是一个meta-algorithm,并且可以与其他一起使用
*学习算法来改进他们的表现。在实践中,演算法
*决策树可能是最受欢迎的组合。演算法是自适应
*,后续分类器建立在有利的调整
*由以前的分类器分类错误实例。演很敏感
*噪声数据和异常值。然而在一些问题它可以不那么容易受到影响
*过度学习问题比大多数学习算法
...
public class AdaBoost implements Classifier<double[]> {
    ...
    /**
     * 变量的重要性。每次分裂了变量的节点(基尼、信息增益等)杂质标准的两个后代节点小于父节点。增加减少为每个变量对所有树木在森林里给一个快变量重要性,往往是非常符合重要性排列的措施
    ...
    public AdaBoost(Attribute[] attributes, double[][] x, int[] y, int T, int J) {
        ...
        int[][] order = MlUtils.sort(attributes, x);
        ...
        if (k == 2) {
            ...
            for (int i = 0; i < T; i++) {
                for (int j = 0; j < n; j++) {
                    ...
                    label[j] = MathBase.whichMax(prediction[j]);
                }
