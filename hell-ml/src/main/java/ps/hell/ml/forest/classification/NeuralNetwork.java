...
/**
 * 多层感知器神经网络。
*一个中长期规划由几层节点,通过加权实现连接
*无环弧前从每个层以下,没有横向或
*反馈连接。每个节点计算一个加权线性转换
*组合的输入(输出激活前层)
*的权重作为连接到一个恒定的输入可教育的偏见。
*转换,称为激活函数,是一个有界的引入
*(非线性)功能,如乙状结肠功能(范围从0到1)。
*另一种流行的活动实际上是双曲正切函数
*相当于乙状结肠函数形状,但范围从1到1。
*更专门的激活功能包括径向基函数
*用于RBF网络
...
public class NeuralNetwork implements OnlineClassifier<double[]> {
        ...
        for (int l = 1; l < numLayers; l++) {
            ...
            for (int i = 0; i < net[l].units; i++) {
                for (int j = 0; j <= net[l - 1].units; j++) {
                    net[l].weight[i][j] = MathBase.random(-r, r);
                }
            }
        }
    }
    ...
    public NeuralNetwork clone() {
        ...
        for (int i = 0; i < numLayers; i++) {
            ...
            if (i > 0) {
                copycat.net[i].weight = MathBase.clone(net[i].weight);
                copycat.net[i].delta = MathBase.clone(net[i].delta);
            }
        }
    ...
    private void propagate(Layer lower, Layer upper) {
        for (int i = 0; i < upper.units; i++) {
            ...
            if (upper != outputLayer || activationFunction == ActivationFunction.LOGISTIC_SIGMOID) {
                upper.output[i] = MathBase.logistic(sum);
            } else {
