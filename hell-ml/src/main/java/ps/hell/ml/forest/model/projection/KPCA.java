...
public class KPCA<T> implements Projection<T> {
    ...
    public KPCA(T[] data, MercerKernel<T> kernel, int k, double threshold) {
        ...
        EigenValueDecomposition2 eigen = MathBase.eigen(K, k);
        ...
        for (int i = 0; i < n; i++) {
            MathBase.ax(projection, K[i], coordinates[i]);
        }
    }
    ...
    public double[] project(T x) {
        ...
        double my = MathBase.mean(y);
        for (int i = 0; i < n; i++) {
            y[i] = y[i] - my - mean[i] + mu;
        }

        double[] z = new double[p];
        MathBase.ax(projection, y, z);
        return z;
    }
    ...
    public double[][] project(T[] x) {
        ...
        for (int i = 0; i < y.length; i++) {
            MathBase.ax(projection, y[i], z[i]);
        }
        return z;
    }
}
