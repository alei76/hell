...
package ps.hell.ml.nlp.tool.aliasi.dca;
...
public class DiscreteChooser
    ...
    public double[] choiceLogProbs(Vector[] choices) {
        ...
        double Z = ps.hell.ml.nlp.tool.aliasi.util.Math.logSumOfExponentials(scores);
        for (int i = 0; i < choices.length; ++i)
            scores[i] -= Z;
        // scores are log probs
        return scores;
    ...
    public static DiscreteChooser
        ...
        for (int epoch = 0; epoch < maxEpochs; ++epoch) {
            ...
            double lp
                = ps.hell.ml.nlp.tool.aliasi.util.Math.logBase2ToNaturalLog(prior.log2Prior(coefficientVector));
            double llp = ll + lp;
            if (llp > bestLlp) {
                bestLlp = llp;
            }
            if (epoch > 0) {
                double relativeDiff
                    = ps.hell.ml.nlp.tool.aliasi.util.Math.relativeAbsoluteDifference(lastLlp,llp);
