...
class CompiledHmmCharLm extends AbstractHmm {
    ...
    public CompiledHmmCharLm(ObjectInput in) 
        ...
        for (int i = 0; i < numStates; ++i)
            for (int j = 0; j < numStates; ++j)
                mTransitionLog2Probs[i][j]
                    = ps.hell.ml.nlp.tool.aliasi.util.Math.log2(mTransitionProbs[i][j]
                                                = in.readDouble());
        ...
        for (int i = 0; i < numStates; ++i)
            mStartLog2Probs[i] 
                = ps.hell.ml.nlp.tool.aliasi.util.Math.log2(mStartProbs[i] = in.readDouble());
        for (int i = 0; i < numStates; ++i)
            mEndLog2Probs[i] 
                = ps.hell.ml.nlp.tool.aliasi.util.Math.log2(mEndProbs[i] = in.readDouble());
