...
import ps.hell.ml.nlp.tool.aliasi.matrix.Matrices;
...
import ps.hell.ml.nlp.tool.aliasi.tag.*;
import ps.hell.ml.nlp.tool.aliasi.util.*;
 ...
 * ps.hell.ml.nlp.tool.aliasi.classify.LogisticRegressionClassifier} and {@link
 * ps.hell.ml.nlp.tool.aliasi.stats.LogisticRegression}.  It's mathematically
...
public class ChainCrf<E>
     ...
     * not.
     *
     * @param tags Array of output tags. 标签
     * @param coefficients Array of coefficient vectors parallel to tags. 系数
    ...
    TagLattice<E> forwardBackward(List<E> tokens,
        ...
        for (int nTo = 1; nTo < numTokens; ++nTo) {
            for (int kTo = 0; kTo < numTags; ++kTo) {
                ...
                logForwards[nTo][kTo] = ps.hell.ml.nlp.tool.aliasi.util.Math.logSumOfExponentials(buf);
            }
        }
        ...
        for (int nFrom = numTokens-1; --nFrom >= 0; ) {
            for (int kFrom = 0; kFrom < numTags; ++kFrom) {
                ...
                logBackwards[nFrom][kFrom] = ps.hell.ml.nlp.tool.aliasi.util.Math.logSumOfExponentials(buf);
            }
        }

        double logZ = ps.hell.ml.nlp.tool.aliasi.util.Math.logSumOfExponentials(logForwards[numTokens-1]);
        ...
        double logZ() {
            ...
            for (int n = 0; n < mTransitionScores.length; ++n) {
                ...
                for (int k = 0; k < numTags; ++k) {
                    ...
                    forwards[k] = ps.hell.ml.nlp.tool.aliasi.util.Math.logSumOfExponentials(exps);
                }
            }
            double logZ = ps.hell.ml.nlp.tool.aliasi.util.Math.logSumOfExponentials(forwards);
            return logZ;
        }
    ...
    public static <F> ChainCrf<F>
        ...
        for (int epoch = 0; epoch < maxEpochs; ++epoch) {
            ...
            double relativeDiff = ps.hell.ml.nlp.tool.aliasi.util.Math
