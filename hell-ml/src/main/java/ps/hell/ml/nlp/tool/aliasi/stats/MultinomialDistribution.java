...
public class MultinomialDistribution {
    ...
    public double chiSquared(int[] sampleCounts) {
        checkNumSamples(sampleCounts);
        int totalCount = ps.hell.ml.nlp.tool.aliasi.util.Math.sum(sampleCounts);
        double sum = 0.0;
        for (int i = 0; i < sampleCounts.length; ++i)
    ...
    public static double log2MultinomialCoefficient(int[] sampleCounts) {
        ...
        long totalCount = ps.hell.ml.nlp.tool.aliasi.util.Math.sum(sampleCounts);
        double coeff = ps.hell.ml.nlp.tool.aliasi.util.Math.log2Factorial(totalCount);
        for (int i = 0; i < sampleCounts.length; ++i)
            coeff -= ps.hell.ml.nlp.tool.aliasi.util.Math.log2Factorial(sampleCounts[i]);
        return coeff;
    }
