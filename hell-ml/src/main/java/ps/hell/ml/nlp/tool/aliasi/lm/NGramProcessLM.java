...
package ps.hell.ml.nlp.tool.aliasi.lm;
...
import ps.hell.ml.nlp.tool.aliasi.stats.Model;
...
public class NGramProcessLM
     ...
     * length.
     *
     * @param maxNGram Maximum length n-gram for which counts are 对应的ngram的数量
     * stored.
     */
    ...
    public double log2ConditionalEstimate(char[] cs, int start, int end,
        ...
        return ps.hell.ml.nlp.tool.aliasi.util.Math.log2(currentEstimate);
    }
    ...
    public final void setNumChars(int numChars) {
        ...
        mLog2UniformEstimate
            = ps.hell.ml.nlp.tool.aliasi.util.Math.log2(mUniformEstimate);
    }
    ...
    static class Externalizer extends AbstractExternalizable {
        ...
        public void writeExternal(ObjectOutput dataOut) throws IOException {
            ...
            float log2OneMinusLambda = Double.isNaN(oneMinusLambda)
                ? 0f : (float) ps.hell.ml.nlp.tool.aliasi.util.Math.log2(oneMinusLambda);
            ...
            for (int index = 1; !queue.isEmpty(); ++index) {
                ...
                if (index <= lastInternalNodeIndex) {
                    ...
                    float log2OneMinusLambda2
                        = (float) ps.hell.ml.nlp.tool.aliasi.util.Math.log2(oneMinusLambda2);
                ...
                for (int i = 0; i < cs2.length; ++i)
                    queue.add(ps.hell.ml.nlp.tool.aliasi.util.Arrays.concatenate(nGram,cs2[i]));
            }
        }
    }
