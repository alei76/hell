...
import ps.hell.ml.nlp.tool.aliasi.crf.ForwardBackwardTagLattice;
import ps.hell.ml.nlp.tool.aliasi.symbol.SymbolTable;
import ps.hell.ml.nlp.tool.aliasi.tag.StringTagging;
...
public class BioTagChunkCodecTest {
    ...
    TagLattice randomLattice(Set<String> chunkTypes,
        ...
        for (int i = 1; i < tokens.size(); ++i) {
            for (int j = 0; j < tags.size(); ++j) {
                ...
                logForwards[i][j] = ps.hell.ml.nlp.tool.aliasi.util.Math.logSumOfExponentials(basis);
            }
        }
        double logZ = ps.hell.ml.nlp.tool.aliasi.util.Math.logSumOfExponentials(logForwards[logForwards.length-1]);
        ...
        for (int i = tokens.size()-1; --i >= 0; ) {
            for (int j = 0; j < tags.size(); ++j) {
                ...
                logBackwards[i][j] = ps.hell.ml.nlp.tool.aliasi.util.Math.logSumOfExponentials(basis);
            }
        }
    ...
    List<Chunk> bruteForce(TagLattice<String> lattice,
        ...
        double totalProb = ps.hell.ml.nlp.tool.aliasi.util.Math.logSumOfExponentials(logScoreArray);
        // System.out.println("\nTOTAL=" + totalProb);
        ...
        for (Map.Entry<Chunk,List<Double>> entry : chunkToScores.entrySet()) {
            ...
            for (int i = 0; i < xs.length; ++i)
                xs[i] = vals.get(i);
            double logSumExp = ps.hell.ml.nlp.tool.aliasi.util.Math.logSumOfExponentials(xs);
