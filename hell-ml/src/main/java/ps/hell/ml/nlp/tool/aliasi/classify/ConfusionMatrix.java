...
public class ConfusionMatrix {
    ...
    public double jointEntropy() {
        ...
        for (int i = 0; i < numCategories(); ++i) {
            for (int j = 0; j < numCategories(); ++j) {
                ...
                entropySum += prob * ps.hell.ml.nlp.tool.aliasi.util.Math.log2(prob);
            }
        }
        return -entropySum;
    }
    ...
    public double conditionalEntropy(int refCategoryIndex) {
        ...
        for (int j = 0; j < numCategories(); ++j) {
            ...
            entropySum += conditionalProb
                * ps.hell.ml.nlp.tool.aliasi.util.Math.log2(conditionalProb);
        }
        return -entropySum;
    }
    ...
    public double mutualInformation() {
        ...
        for (int i = 0; i < numCategories(); ++i) {
            ...
            for (int j = 0; j < numCategories(); ++j) {
                ...
                if (pIJ <= 0.0) continue;
                sum += pIJ * ps.hell.ml.nlp.tool.aliasi.util.Math.log2(pIJ/(pI * pJ));
            }
        }
        return sum;
    }
    ...
    public double klDivergence() {
        ...
        for (int k = 0; k < numCategories(); ++k) {
            ...
            double responseProb = eval.responseLikelihood();
            sum += refProb
                * ps.hell.ml.nlp.tool.aliasi.util.Math.log2(refProb/responseProb);
        }
        return sum;
    }
