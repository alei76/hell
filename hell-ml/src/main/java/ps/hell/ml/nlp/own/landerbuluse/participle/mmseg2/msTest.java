 ...
 *
 */
public class msTest implements AnalyzeParticiple {
	/**
	 * 复杂匹配的单词个数
	 */
	 ...
	 *            清除的词库
	 */
	public msTest(String[] usedWords, String[] clearnWords) {
		// 添加基础词库
	 ...
	 */
	public ArrayList<ArrayList<WordFather>> analyze(String inputData) {
		ArrayList<String> input = clearnParams(inputData);
		ArrayList<ArrayList<WordFather>> result = new ArrayList<ArrayList<WordFather>>();
		for (String str : input) {
			ArrayList<WordFather> re = participleMethodMmseg(str);
	 ...
	 * @return 按照某些标点符号将划分开 对应每一个子句子 出Word[]
	 */
	public ArrayList<ArrayList<ArrayList<WordFather>>> analyze(
			ArrayList<String> inputData) {
		ArrayList<ArrayList<ArrayList<WordFather>>> out = new ArrayList<ArrayList<ArrayList<WordFather>>>();
		for (String inputString : inputData) {
			ArrayList<String> input = clearnParams(inputString);
			ArrayList<ArrayList<WordFather>> result = new ArrayList<ArrayList<WordFather>>();
			for (String str : input) {
				ArrayList<WordFather> re = participleMethodMmseg(str);
				if (re == null || re.size() == 0) {
	 ...
	 * @return
	 */
	public ArrayList<ArrayList<ArrayList<WordFather>>> analyze(File file,
			String code) {
		ArrayList<ArrayList<ArrayList<WordFather>>> result = new ArrayList<ArrayList<ArrayList<WordFather>>>();
		if (!file.exists()) {
		...
		try {
			...
			while ((tempString = reader.readLine()) != null) {
				if (tempString.length() > 0) {
					ArrayList<ArrayList<WordFather>> re = analyze(tempString);
					if (re.size() == 0) {
						break;
					} else {
						result.add(re);
	 ...
	 * @return
	 */
	public ArrayList<ArrayList<WordFather>> analyzeEmotion(
			ArrayList<ArrayList<WordFather>> words) {
		ArrayList<ArrayList<WordFather>> result = new ArrayList<ArrayList<WordFather>>();
		while (words.size() > 0) {
			ArrayList<WordFather> temp = words.remove(0);//words.pollFirst();
			ArrayList<WordFather> re = new ArrayList<WordFather>();
			while (temp.size() > 0) {
				WordFather temp2 = temp.remove(0);//temp.pollFirst();
	 ...
	 * @return
	 */
	public LinkedList<LinkedList<WordFather>> clearnStop(LinkedList<LinkedList<WordFather>> inputData) {
	 ...
	 * @return
	 */
	public ArrayList<ArrayList<ArrayList<WordFather>>> clearnStop(
			ArrayList<ArrayList<ArrayList<WordFather>>> inputData) {
		for (ArrayList<ArrayList<WordFather>> wordList : inputData) {
			for (ArrayList<WordFather> words : wordList) {
				Iterator<WordFather> iterator = words.iterator();
				while (iterator.hasNext()) {
					Word word = iterator.next();
					if (wordsClearn.otherWords.contains(word)) {
	 ...
	 * @return
	 */
	public ArrayList<String> clearnParams(String inputData) {
	 ...
	 * @return
	 */
	public ArrayList<WordFather> participleMethodMmseg(String inputData) {
		ArrayList<WordFather> result = new ArrayList<WordFather>();
		// 存储当前chunk单组的个数
		int count = 0;
		// 字符开始位置
		int start = 0;
		...
		while (true) {
			...
			// 获取没一组的全部配比，标准是三组
			// 复杂匹配的输出类型
			ArrayList<ArrayList<WordFather>> comResult = getWords(str);
//			for (ArrayList<WordFather> words : comResult) {
//				for (Word word : words) {
...
//			}
			...
			if (comResult.size() > 1) {
				Iterator<ArrayList<WordFather>> iterator1 = comResult
						.iterator();
				while (iterator1.hasNext()) {
					ArrayList<WordFather> words = iterator1.next();
					index++;
					double len = 0;
					for (Word w : words) {
						len += w.word.length();
			...
			ArrayList<WordFather> en = comResult.get(0);
			index = 0;
			int ll_len = 0;
			// 添加除了最后一位的所有
	 ...
	 * @return
	 */
	public ArrayList<WordFather> sampleAdapt(String str) {
		// if(str.equals("……"))
		// {
		// System.out.println(str);
		// }
	 ...
	 * @return
	 */
	public ArrayList<ArrayList<WordFather>> sampleAdapt2(String str) {
		ArrayList<WordFather> list = sampleAdapt(str);
		ArrayList<ArrayList<WordFather>> result = new ArrayList<ArrayList<WordFather>>();
		for (WordFather word : list) {
			ArrayList<WordFather> one = new ArrayList<WordFather>();
			one.add(word);
			result.add(one);
		}
		return result;
	 ...
	 * @return
	 */
	public ArrayList<ArrayList<WordFather>> getWords(String str) {
		ArrayList<ArrayList<WordFather>> result = null;
	 ...
	 * @return
	 */
	public ArrayList<ArrayList<WordFather>> comAdapt(String str,
			ArrayList<ArrayList<WordFather>> inputList) {
		ArrayList<ArrayList<WordFather>> result = new ArrayList<ArrayList<WordFather>>();
		for (ArrayList<WordFather> list : inputList) {
			int length = 0;
			...
			ArrayList<WordFather> one = sampleAdapt(strTemp);
			for (WordFather word : one) {
				ArrayList<WordFather> temp = ((ArrayList<WordFather>) list
						.clone());
				temp.add(word);
				result.add(temp);
			}
	...
	public static void main(String[] args) {
		...
		msTest mm = new msTest(null, null);
		int i = 0;
		ArrayList<ArrayList<WordFather>> re = mm.analyze(lll);
		for (ArrayList<WordFather> words : re) {
			i++;
			for (Word word : words) {
				if (word.wordProperty == null) {
					System.out.print(word.word + "[]" + "\t");
		...
		//System.exit(1);
		 ...
		 ArrayList<ArrayList<ArrayList<WordFather>>> ma = mm.analyze(file,
		 "gbk");
		 Date end = new Date();
		 ...
		 System.exit(1);
		// for (ArrayList<ArrayList<WordFather>> re : ma) {
		// for (ArrayList<WordFather> words : re) {
		// i++;
		// for (Word word : words) {
		// if (word.wordProperty == null) {
