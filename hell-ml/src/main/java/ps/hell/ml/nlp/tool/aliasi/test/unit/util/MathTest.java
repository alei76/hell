...
package ps.hell.ml.nlp.tool.aliasi.test.unit.util;
...
public class MathTest  {
    ...
    public void testDigamma() {
        for (double[] xFx : DIGAMMA_TESTS) {
            ...
            double val = ps.hell.ml.nlp.tool.aliasi.util.Math.digamma(x); // ,200000);
    ...
    public void testGamma() {
        for (int i = 2; i < 100; ++i)
            assertEquals(ps.hell.ml.nlp.tool.aliasi.util.Math.log2Factorial(i-1),
                         ps.hell.ml.nlp.tool.aliasi.util.Math.log2Gamma(i),
                         0.0001);
        ...
        for (double[] pair : GAMMA_INPUT_NATLOGOUT_TESTS) {
            ...
            double log2GammaZ = ps.hell.ml.nlp.tool.aliasi.util.Math.log2(gammaZ);
            double foundLog2GammaZ = ps.hell.ml.nlp.tool.aliasi.util.Math.log2Gamma(z);
            assertEquals(log2GammaZ,ps.hell.ml.nlp.tool.aliasi.util.Math.log2Gamma(z),0.0001);
        }
    }
    ...
    private static double log2FactorialFull(int n) {
        if (n == 0) return 0;
        return ps.hell.ml.nlp.tool.aliasi.util.Math.log2(n) + log2FactorialFull(n-1);
    }
    ...
    public void testLogSumOfExponentials() {
        ...
        // test overflow
        assertEquals(10000.0, ps.hell.ml.nlp.tool.aliasi.util.Math.logSumOfExponentials(new double[] { 10000.0 }), 0.0001);
    }

    @Test
    public void testMax() {
        assertTrue(Double.isNaN(ps.hell.ml.nlp.tool.aliasi.util.Math.max(new double[] { })));
        assertTrue(Double.isNaN(ps.hell.ml.nlp.tool.aliasi.util.Math.max(new double[] {
                                                             Double.NaN
                                                         })));
        assertTrue(Double.isNaN(ps.hell.ml.nlp.tool.aliasi.util.Math.max(new double[] {
        ...
        assertEquals(1.0,ps.hell.ml.nlp.tool.aliasi.util.Math.max(new double[] { 1.0 }));
        assertEquals(Double.POSITIVE_INFINITY,
                     ps.hell.ml.nlp.tool.aliasi.util.Math.max(new double[] {
                                                  Double.POSITIVE_INFINITY,
                                                  Double.NEGATIVE_INFINITY
                                              }));

        assertEquals(2.0,ps.hell.ml.nlp.tool.aliasi.util.Math.max(new double[] { 1.0, 2.0 }));
        assertEquals(2.0,ps.hell.ml.nlp.tool.aliasi.util.Math.max(new double[] { 2.0, 1.0 }));
        assertEquals(2.0,ps.hell.ml.nlp.tool.aliasi.util.Math.max(new double[] { 2.0, 1.0, -1.0 }));
    }
    ...
    static void assertLogSumOfExponentials(double... xs) {
        ...
        assertEquals(expected,ps.hell.ml.nlp.tool.aliasi.util.Math.logSumOfExponentials(xs),0.0001);
    }
