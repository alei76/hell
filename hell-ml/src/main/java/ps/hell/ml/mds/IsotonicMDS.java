...
/**
 * 克鲁斯卡非度量MDS。在摘要MDS,只有等级次序的条目
*在距离矩阵(而不是实际的相异)是假设
*包含的重要信息。因此,最后的距离
*配置应该尽可能相同的等级次序
*原始数据。注意,一个完美的序数re-scaling数据
*距离通常是不可能的。通常发现的关系
*使用等渗回归
...
public class IsotonicMDS {
    ...
    public IsotonicMDS(double[][] proximity, double[][] init, double tol, int maxIter) {
        ...
        stress = 0.0;
        try {
            stress = MathBase.min(func, 5, x, tol, maxIter);
        } catch (Exception ex) {
            // If L-BFGS doesn't work, let's try BFGS.
            stress = MathBase.min(func, x, tol, maxIter);
        }
        ...
        void dist(double[] x) {
            ...
            for (int i = 0; i < nr; i++) {
                for (int j = i + 1; j < nr; j++) {
                    double tmp = 0.0;
                    for (int c = 0; c < nc; c++) {
                        tmp += MathBase.sqr(x[i * nc + c] - x[j * nc + c]);
                    }
