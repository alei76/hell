...
/**
 * 马斯的映射是一个迭代的技术使得interpoint
*在低维投影距离尽可能接近
* interpoint距离在高维对象。两个点关闭
*在高维空间应该出现在一起
*投影,而高维空间中的两个点远
*出现远的投影。马斯的映射是一个特例
*指标最小二乘多维标度
...
public class SammonMapping {
    ...
    public SammonMapping(double[][] proximity, double[][] init, double lambda, double tol, int maxIter) {
        ...
        coordinates = MathBase.clone(init);
        ...
        for (int i = 0; i < n; i++) {
            for (int j = i + 1; j < n; j++) {
                ...
                double rij = MathBase.distance(coordinates[i], coordinates[j]);
                stress += MathBase.sqr(dij - rij) / dij;
            }
        }
        stress /= c;
        double epast = stress;
        ...
        for (int iter = 1; iter <= maxIter; iter++) {
            ...
            for (int i = 0; i < n; i++) {
                for (int j = i + 1; j < n; j++) {
                    double dij = proximity[i][j];
                    if (dij == 0.0) dij = 1.0E-10;
                    double rij = MathBase.distance(xu[i], xu[j]);
                    stress += MathBase.sqr(dij - rij) / dij;
                }
            }
            stress /= c;
