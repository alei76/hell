...
public class SIB extends PartitionClustering<double[]> {
    ...
    public SIB(double[][] data, int k, int maxIter) {
        ...
        for (int iter = 1, reassignment = n; iter <= maxIter && reassignment > 0; iter++) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                int c = -1;
                for (int j = 0; j < k; j++) {
                    double dist = MathBase.JensenShannonDivergence(data[i], centroids[j]);
                    if (nearest > dist) {
                        nearest = dist;
                        c = j;
                    }
        ...
        distortion = 0;
        for (int i = 0; i < n; i++) {
            distortion += MathBase.JensenShannonDivergence(data[i], centroids[y[i]]);
        }            
    }
    ...
    private static int[] seed(SparseDataset data, int k) {
        ...
        int[] y = new int[n];
        SparseArray centroid = data.get(MathBase.randomInt(n)).x;
        ...
        for (int i = 1; i < k; i++) {
            for (int j = 0; j < n; j++) {
                double dist = MathBase.JensenShannonDivergence(data.get(j).x, centroid);
                if (dist < D[j]) {
                    D[j] = dist;
                    y[j] = i - 1;
                }
            ...
            double cutoff = MathBase.random() * MathBase.sum(D);
            double cost = 0.0;
            int index = 0;
            for (; index < n; index++) {
                cost += D[index];
    ...
    public SIB(SparseDataset data, int k, int maxIter) {
        ...
        for (int iter = 1, reassignment = n; iter <= maxIter && reassignment > 0; iter++) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                int c = -1;
                for (int j = 0; j < k; j++) {
                    double dist = MathBase.JensenShannonDivergence(data.get(i).x, centroids[j]);
                    if (nearest > dist) {
                        nearest = dist;
                        c = j;
                    }
        ...
        distortion = 0;
        for (int i = 0; i < n; i++) {
            distortion += MathBase.JensenShannonDivergence(data.get(i).x, centroids[y[i]]);
        }            
    }
    ...
    public int predict(double[] x) {
        ...
        for (int i = 0; i < k; i++) {
            double dist = MathBase.JensenShannonDivergence(x, centroids[i]);
            if (dist < minDist) {
                minDist = dist;
                bestCluster = i;
            }
