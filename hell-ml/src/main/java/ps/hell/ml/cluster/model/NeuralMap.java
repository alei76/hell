...
public class NeuralMap implements Clustering<double[]> {
        ...
        Neighbor nearest(double[] x) {
            ...
            for (int i = 0; i < hash.length; i++) {
                ...
                if (bin != null) {
                    for (Hash.Item e : bin) {
                        if (e.bucket == bucket) {
                            double distance = MathBase.distance(x, e.neuron.w);
    ...
    public void update(double[] x) {
        ...
        for (Iterator<Neuron> iter = neuron.neighbors.iterator(); iter.hasNext(); ) {
            ...
            if (MathBase.distance(neuron.w, neighbor.w) > 2 * r) {
    ...
    public int partition(int k, int minPts) {
        ...
        for (int i = 0; i < data.size(); i++) {
            ...
            for (int j = 0; j < i; j++) {
                proximity[i][j] = MathBase.distance(data.get(i).w, data.get(j).w);
            }
        }
    ...
    public int predict(double[] x) {
        ...
        for (Neuron neuron : neurons) {
            double dist = MathBase.squaredDistance(x, neuron.w);
            if (dist < minDist) {
                minDist = dist;
