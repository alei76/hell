...
import ps.hell.ml.cluster.model.linkage.Linkage;
...
public class SOM implements Clustering<double[]> {
    ...
    public SOM(double[][] data, int width, int height) {
        ...
        EigenValueDecomposition2 eigen = MathBase.eigen(V, 2);
        ...
        for (int i = 0; i < height - 1; i++) {
            for (int j = 0; j < width - 1; j++) {
                double dist = MathBase.sqrt(MathBase.squaredDistance(neurons[i][j], neurons[i][j + 1]));
                umatrix[i][j] = Math.max(umatrix[i][j], dist);
                umatrix[i][j + 1] = Math.max(umatrix[i][j + 1], dist);
                dist = Math.sqrt(MathBase.squaredDistance(neurons[i][j], neurons[i + 1][j]));
                umatrix[i][j] = Math.max(umatrix[i][j], dist);
        ...
        for (int i = 0; i < height - 1; i++) {
            double dist = Math.sqrt(MathBase.squaredDistance(neurons[i][width - 1], neurons[i + 1][width - 1]));
            umatrix[i][width - 1] = Math.max(umatrix[i][width - 1], dist);
            umatrix[i + 1][width - 1] = Math.max(umatrix[i + 1][width - 1], dist);
        }

        for (int j = 0; j < width - 1; j++) {
            double dist = Math.sqrt(MathBase.squaredDistance(neurons[height - 1][j], neurons[height - 1][j + 1]));
    ...
    public int[] partition(int k) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < i; j++) {
                proximity[i][j] = MathBase.distance(units[i], units[j]);
            }
        }
    ...
    public int predict(double[] x) {
        ...
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                double dist = MathBase.squaredDistance(neurons[i][j], x);
                if (dist < best) {
                    best = dist;
                    ii = i;
                    jj = j;
