...
public class DeterministicAnnealing extends KMeans {
    ...
    public DeterministicAnnealing(double[][] data, int Kmax, double alpha) {
        ...
        double[][] cov = MathBase.cov(data, centroids[0]);
        double[] ev = new double[d];
        Arrays.fill(ev, 1.0);
        double lambda = MathBase.eigen(cov, ev, 1E-4);
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < k; j += 2) {
                double dist = MathBase.squaredDistance(data[i], centroids[j]);
                if (nearest > dist) {
                    y[i] = j / 2;
                    nearest = dist;
                }
        ...
        while (iter < 100 && currentDistortion > newDistortion) {
            ...
            if (Double.isNaN(D)) {
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    for (int j = 0; j < k; j++) {
                        dist[j] = MathBase.squaredDistance(data[i], centroids[j]);
