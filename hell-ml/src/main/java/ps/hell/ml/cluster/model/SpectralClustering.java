...
public class SpectralClustering {
    ...
    public SpectralClustering(double[][] W, int k) {
        ...
        EigenValueDecomposition2 eigen = MathBase.eigen(L, k);
        double[][] Y = eigen.getEigenVectors();
        for (int i = 0; i < n; i++) {
            MathBase.unitize2(Y[i]);
        }
    ...
    public SpectralClustering(double[][] data, int k, double sigma) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < i; j++) {
                W[i][j] = MathBase.exp(gamma * MathBase.squaredDistance(data[i], data[j]));
                W[j][i] = W[i][j];
            }
        }
    ...
    public SpectralClustering(double[][] data, int l, int k, double sigma) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < n; j++) {
                if (i != j) {
                    double w = MathBase.exp(gamma * MathBase.squaredDistance(data[i], data[j]));
                    sum += w;
                    if (j < l) {
                        C[i][j] = w;
                    }
        ...
        EigenValueDecomposition2 eigen = MathBase.eigen(W, k);
