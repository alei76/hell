...
import ps.hell.ml.neighbor.RNNSearch;
...
public class MEC <T> extends PartitionClustering<T> {
            ...
            for (int i = 0; i < n; i++) {
                if (mostSignificantNeighborhoodCluster[i] != y[i]) {
                    ...
                    for (int neighbor : neighbors.get(i)) {
                        ...
                        if (r1 > 0) {
                            oldMutual -= r1 * MathBase.log2(r1) * px[neighbor];
                        }
                        if (r2 > 0) {
                            oldMutual -= r2 * MathBase.log2(r2) * px[neighbor];
                        }
                        ...
                        if (r1 > 0) {
                            newMutual -= r1 * MathBase.log2(r1) * px[neighbor];
                        }
                        if (r2 > 0) {
                            newMutual -= r2 * MathBase.log2(r2) * px[neighbor];
                        }
                    }
    ...
    public int predict(T x) {
        ...
        return MathBase.whichMax(label);
    }
