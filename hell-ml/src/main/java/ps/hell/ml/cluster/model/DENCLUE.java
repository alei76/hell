    ...
    public DENCLUE(double[][] data, double sigma, int m) {
        ...
        if (n >= 1000 && np >= 2) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                while (diff > eps) {
                    double weight = 0.0;
                    for (int j = 0; j < m; j++) {
                        double w = MathBase.exp(gamma * MathBase.squaredDistance(attractors[i], samples[j]));
                        weight += w;
                        for (int l = 0; l < d; l++) {
                    ...
                    if (diff > 1E-5) {
                        radius[i] = 2 * MathBase.distance(attractors[i], attractor);
                    }
        ...
        for (int i = 1; i < n; i++) {
            ...
            for (int j = 0; j < cluster.size(); j++) {
                if (MathBase.distance(attractors[i], cluster.get(j)) < radius[i] + step.get(j)) {
                    y[i] = j;
                    newcluster = false;
    ...
    public int predict(double[] x) {
        ...
        while (diff > eps) {
            ...
            for (int i = 0; i < samples.length; i++) {
                double w = MathBase.exp(gamma * MathBase.squaredDistance(samples[i], z));
                weight += w;
                for (int j = 0; j < p; j++) {
            ...
            if (diff > 1E-5) {
                step = 2 * MathBase.distance(attractor, z);
            }
        ...
        for (int i = 0; i < k; i++) {
            if (MathBase.distance(attractors[i], z) < radius[i] + step) {
                return i;
            }
        }
