...
/**
 * 一阶隐马尔可夫模型。一个隐藏的马尔可夫模型(HMM)是一个统计
*马尔可夫模型的系统被建模为一个马尔可夫
*过程的(隐藏)。嗯可以被认为是
*简单的动态贝叶斯网络。< p >在常规的马尔可夫模型中,状态
*直接可见的观察者,因此状态转换
*概率是唯一的参数。在一个隐藏的马尔可夫模型中,状态
*不是直接可见的,但输出,依赖于国家,是可见的。每一个
*国家对可能的输出具有概率分布令牌。
*因此令牌的顺序由一个嗯给一些信息
*对序列的状态
...
public class HMM<O> implements SequenceLabeler<O> {
    ...
    public HMM(int[][] observations, int[][] labels) {
        ...
        for (int i = 0; i < observations.length; i++) {
            ...
            numStates = Math.max(numStates, MathBase.max(labels[i]) + 1);
            numSymbols = Math.max(numSymbols, MathBase.max(observations[i]) + 1);
        }
        ...
        MathBase.unitize1(pi);
        for (int i = 0; i < numStates; i++) {
            MathBase.unitize1(a[i]);
            MathBase.unitize1(b[i]);
        }
    }
    ...
    public HMM(O[][] observations, int[][] labels) {
        ...
        for (int i = 0; i < obs.length; i++) {
            numStates = Math.max(numStates, MathBase.max(labels[i]) + 1);
            numSymbols = Math.max(numSymbols, MathBase.max(obs[i]) + 1);
        }
