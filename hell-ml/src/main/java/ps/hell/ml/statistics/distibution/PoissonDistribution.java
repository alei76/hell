...
public class PoissonDistribution extends DiscreteDistribution implements DiscreteExponentialFamily {
    ...
    public double p(int k) {
        if (k < 0) {
            ...
            return Math.pow(lambda, k) * Math.exp(-lambda) / MathBase.factorial(k);
        }
    }
    ...
    public double logp(int k) {
        if (k < 0) {
            ...
            return k * Math.log(lambda) - lambda - MathBase.logFactorial(k);
        }
    }
    ...
    class ModeSearch implements RandomNumberGenerator {
        ...
        ModeSearch() {
            ...
            f0Mode = Math.exp(mode * Math.log(lambda) - lambda - MathBase.logFactorial(mode));
        }
    ...
    class Patchwork implements RandomNumberGenerator {
        ...
        Patchwork() {
            ...
            c_pm = mode * l_my - MathBase.logFactorial(mode);
        ...
        public int rand() {
            ...
            for (;;) {
                ...
                if (Math.log(W) <= X * l_my - MathBase.logFactorial(X) - c_pm) {
                    return X;
                }
            }
        }
        ...
        private double f(int k, double l_nu, double c_pm) {
            return Math.exp(k * l_nu - MathBase.logFactorial(k) - c_pm);
        }
    }
