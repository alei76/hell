...
import ps.hell.math.base.matrix.decomposition.CholeskyDecomposition;
...
public class MultivariateGaussianDistribution extends AbstractMultivariateDistribution implements MultivariateExponentialFamily {
    ...
    private void init() {
        dim = mu.length;
        CholeskyDecomposition cholesky = new CholeskyDecomposition(new Matrix(sigma));
        sigmaInv = cholesky.inverseCholesky().data;
        sigmaDet = cholesky.getDeterminant();
        sigmaL = cholesky.getL().data;
    ...
    public double logp(double[] x) {
        ...
        double[] v = x.clone();
       v= MathBase.minus(v, mu);
        double result = MathBase.xax(sigmaInv, v) / -2.0;
    ...
    public double cdf(double[] x) {
        ...
        for (N = 1; err > errMax && N <= Nmax; N++) {
            double[] w = MathBase.random(dim - 1);
            for (int i = 1; i < dim; i++) {
