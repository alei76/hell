...
public class MultivariateGaussianMixture extends MultivariateExponentialFamilyMixture {
     ...
     * @param data the training data.
     * @param k the number of components.
     * @param diagonal true if the components have diagonal covariance matrix.如果组件有对角协方差矩阵对角真实
     */
    public MultivariateGaussianMixture(double[][] data, int k, boolean diagonal) {
        ...
        double[] centroid = data[MathBase.randomInt(n)];
        ...
        for (int i = 1; i < k; i++) {
            ...
            for (int j = 0; j < n; j++) {
                ...
                double dist = MathBase.squaredDistance(data[j], centroid);
                if (dist < D[j]) {
                    D[j] = dist;
                }
            }

            double cutoff = Math.random() * MathBase.sum(D);
            double cost = 0.0;
            int index = 0;
            for (; index < n; index++) {
                cost += D[index];
