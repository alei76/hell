...
import ps.hell.ml.forest.classification.Classifier;
import ps.hell.ml.forest.classification.ClassifierTrainer;
import ps.hell.ml.forest.regression.Regression;
import ps.hell.ml.forest.regression.RegressionTrainer;
...
public class Validation {
    ...
    public static <T> double loocv(ClassifierTrainer<T> trainer, T[] x, int[] y) {
        ...
        for (int i = 0; i < n; i++) {
            T[] trainx = MathBase.slice(x, loocv.train[i]);
    ...
    public static <T> double loocv(RegressionTrainer<T> trainer, T[] x, double[] y) {
        ...
        for (int i = 0; i < n; i++) {
            T[] trainx = MathBase.slice(x, loocv.train[i]);
            double[] trainy = MathBase.slice(y, loocv.train[i]);
            
            Regression<T> model = trainer.train(trainx, trainy);

            rmse += MathBase.sqr(model.predict(x[loocv.test[i]]) - y[loocv.test[i]]);
        }
        
        return MathBase.sqrt(rmse / n);
    }
    ...
    public static <T> double cv(int k, ClassifierTrainer<T> trainer, T[] x, int[] y) {
        ...
        for (int i = 0; i < k; i++) {
            T[] trainx = MathBase.slice(x, cv.train[i]);
            int[] trainy = MathBase.slice(y, cv.train[i]);
    ...
    public static <T> double cv(int k, RegressionTrainer<T> trainer, T[] x, double[] y) {
        ...
        for (int i = 0; i < k; i++) {
            T[] trainx = MathBase.slice(x, cv.train[i]);
            double[] trainy = MathBase.slice(y, cv.train[i]);
    ...
    public static <T> double[] bootstrap(int k, ClassifierTrainer<T> trainer, T[] x, int[] y) {
        ...
        for (int i = 0; i < k; i++) {
            T[] trainx = MathBase.slice(x, bootstrap.train[i]);
            int[] trainy = MathBase.slice(y, bootstrap.train[i]);
    ...
    public static <T> double[] bootstrap(int k, RegressionTrainer<T> trainer, T[] x, double[] y) {
        ...
        for (int i = 0; i < k; i++) {
            T[] trainx = MathBase.slice(x, bootstrap.train[i]);
            double[] trainy = MathBase.slice(y, bootstrap.train[i]);
