...
public class Distributions {
	...
	public static double nextBurr1(double r, int nr,
		 ...
		 ******************************************************************
		 * * FUNCTION : - burr1 samples a random number from one of the * Burr
		 * II, VII, VIII, X distributions with * parameter r > 0 , where the no.
		 * of the * distribution is indicated by a pointer * variable. *
		 * REFERENCE : - L. Devroye (1986): Non-Uniform Random Variate *
		 * Generation, Springer Verlag, New York. * SUBPROGRAM : - drand(seed)
	 ...
	 * <p>
	 * 
	 * @returns a number in the open unit interval <code>(0.0,1.0)</code>
	...
	/**
	 * Returns a discrete geometric distributed random number; <A HREF=
	 * "http://www.statsoft.com/textbook/glosf.html#Geometric Distribution"
	 * >Definition</A>.
	 * <p>
	 * <tt>p(k) = p * (1-p)^k</tt> for <tt> k &gt;= 0</tt>.
	 * <p>
	...
	public static int nextGeometric(double p, RandomEngine randomGenerator) {
		 ...
		 * Inversion normally sequential search is necessary, but in the * case
		 * of the Geometric distribution a direct transformation is * possible
		 * because of the special parallel to the continuous * Exponential
		 * distribution Exp(t): * X - Exp(t): G(x)=1-exp(-tx) * Geo(p):
		 * pk=G(k+1)-G(k)=exp(-tk)*(1-exp(-t)) * p=1-exp(-t) * A random number
		 * of the Geometric distribution Geo(p) is * obtained by k=(long int)x,
	...
	public static double nextLambda(double l3, double l4,
		...
		double x = l_sign
				* (Math.exp(Math.log(u) * l3) - Math
						.exp(Math.log(1.0 - u) * l4));
		return x;
	}
	...
	public static double nextTriangular(RandomEngine randomGenerator) {
		 ...
		 ******************************************************************
		 * * FUNCTION : - tra samples a random number from the * standard
		 * Triangular distribution in (-1,1) * SUBPROGRAM : - drand(seed) ...
		 * (0,1)-Uniform generator with * unsigned long integer *seed. * *
		 ******************************************************************/
		...
		if (u <= 0.5)
			...
			return (1.0 - Math.sqrt(2.0 * (1.0 - u))); /* 0 <= x <= 1 */
	}
	...
	public static double nextWeibull(double alpha, double beta,
		...
		return Math.pow(beta * (-Math.log(1.0 - randomGenerator.raw())),
				1.0 / alpha);
	}
	 ...
	 * @returns a zipfian distributed number in the closed interval
	 *          <tt>[1,Integer.MAX_VALUE]</tt>.
	 */
	public static int nextZipfInt(double z, RandomEngine randomGenerator) {
		/*
		 * Algorithm from page 551 of: Devroye, Luc (1986) `Non-uniform random
		 * variate generation', Springer-Verlag: Berlin. ISBN 3-540-96305-7
		 * (also 0-387-96305-7)
		 */
