 ...
 */
package ps.hell.ml.statistics.base.value;

import ps.hell.ml.statistics.base.value.engine.RandomEngine;
 ...
 * "http://sourceware.cygnus.com/cgi-bin/cvsweb.cgi/gsl/randist/discrete.c?cvsroot=gsl"
...
public class EmpiricalWalker extends AbstractDiscreteDistribution {
	 ...
	 * O(K^2) preprocessing will generally improve that figure of merit,
	 * compared to my cheaper O(K) method; from some experiments with a perl
	 * script, I get values of around 0.6 for my method and just under 0.75 for
	 * Walker's. Knuth has pointed out that finding _the_ optimum lookup tables,
	 * which maximize the average F[k], is a combinatorially difficult problem.
	 * But any valid preprocessing will still provide O(1) time for the call to
	 * gsl_ran_discrete(). I find that if I artificially set F[k]=1 -- ie,
	 * better than optimum! -- I get a speedup of maybe 20%, so that's the
	 ...
	 * "borrows" from the big just enough to bring the small up to mean. This
	 * reduces the size of the big, so the (smaller) big is compared again to
	 * the mean, and if it is smaller, it gets "popped" from the big stack and
	 ...
	 * reference is: G. Marsaglia, Generating discrete random numbers in a
	 * computer, Comm ACM 6, 37-38 (1963). If anybody is interested, I (jt) have
	 * also coded up this version as part of another software package. However,
	 ...
	 */
	public EmpiricalWalker(double[] pdf, int interpolationType,
	...
	public void setState2(double[] pdf) {
		...
		// #if 0
		/*
		 * if 1, then artificially set all F[k]'s to unity. This will give wrong
		 * answers, but you'll get them faster. But, not that much faster (I get
		 * maybe 20%); that's an upper bound on what the optimal preprocessing
		 * would give.
		 */
		/*
		 * for (k=0; k<size; ++k) { F[k] = 1.0; } //#endif
		 */

		// #if KNUTH_CONVENTION
		/* For convenience, set F'[k]=(k+F[k])/K */
		/*
		 * This saves some arithmetic in gsl_ran_discrete(); I find that it
		 * doesn't actually make much difference.
		 */
		/*
		 * for (k=0; k<size; ++k) { F[k] += k; F[k] /= size; } #endif
		 */
		/*
