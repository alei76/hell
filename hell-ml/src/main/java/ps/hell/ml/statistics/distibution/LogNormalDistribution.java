...
import ps.hell.ml.statistics.base.special.Erf;

/**
 * 对数正态分布分布是一个随机变量的概率分布
*的对数正态分布。对数正态分布分布是single-tailed概率分布
*任何随机变量的对数正态分布。如果X是一个
*与正态分布随机变量,然后Y = exp(X)的对数正态分布
*分布;同样,如果Y是对数正态分布分布,然后日志(Y)
正态分布。
*一个变量可以建模为对数正态分布,如果它可以被认为是
*的乘法产品很多独立随机变量的每一个
*这是积极的
...
public class LogNormalDistribution extends AbstractDistribution {
    ...
    public LogNormalDistribution(double[] data) {
        ...
        this.mu = MathBase.mean(x);
        this.sigma = MathBase.sd(x);
