...
public class PoissonSlow extends AbstractDiscreteDistribution {
	...
	protected static final double MEAN_MAX = Integer.MAX_VALUE; // for all means
																...
																// but return
																// the mean.
	protected static final double SWITCH_MEAN = 12.0; // switch from method A to
														// method B
	...
	protected static PoissonSlow shared = new PoissonSlow(0.0,
	...
	protected int nextIntSlow() {
		...
		if (product <= 0.0 && bound > 0.0)
			return (int) Math.round(mean); // detected endless loop due to
											// rounding errors
		return count - 1;
	}
