 ...
 * <dt>
 * High performance implementation. This is a port of <A HREF=
 * "http://wwwinfo.cern.ch/asd/lhc++/clhep/manual/RefGuide/Random/RandChiSquare.html"
...
public class ChiSquare extends AbstractContinousDistribution {
	...
	protected static ChiSquare shared = new ChiSquare(1.0,
	...
	public double nextDouble(double freedom) {
		 ...
		 * distribution with parameter a > 1. * REFERENCE : - J.F. Monahan
		 * (1987): An algorithm for * generating chi random variables, ACM
		 * Trans. * Math. Software 13, 168-172. * SUBPROGRAM : - anEngine ...
		 * pointer to a (0,1)-Uniform * engine * * Implemented by R. Kremer,
		 * 1990 *
		 ******************************************************************/
		...
		if (freedom == 1.0) {
			...
			for (;;) {
				...
				if (zz > (1.036961043 / u + 1.4))
					continue;
				if (2.0 * Math.log(u) < (Math.log(1.0 + z / b) * b * b - zz
						* 0.5 - z * b))
					return ((z + b) * (z + b));
			}
		}
	...
	public double pdf(double x) {
		...
		return Math.exp((freedom / 2.0 - 1.0) * Math.log(x / 2.0) - x / 2.0
				- logGamma) / 2.0;
	}
