...
package ps.hell.ml.statistics.distibution.hypothesis;
...
public class TTest {
    ...
    public static TTest test(double[] x, double mean) {
        ...
        double mu = MathBase.mean(x);
        double var = MathBase.var(x);
    ...
    public static TTest test(double[] x, double[] y, boolean equalVariance) {
        if (equalVariance) {
            ...
            double mu1 = MathBase.mean(x);
            double var1 = MathBase.var(x);

            double mu2 = MathBase.mean(y);
            double var2 = MathBase.var(y);
            //如果为等方差方法则直接获取自由度
            int df = n1 + n2 - 2;
            ...
            double var2 = MathBase.var(y);
            //获取基于方差平均自由度
            double df = MathBase.sqr(var1 / n1 + var2 / n2) / (MathBase.sqr(var1 / n1) / (n1 - 1) + MathBase.sqr(var2 / n2) / (n2 - 1));
            //做正规化
