...
import ps.hell.ml.statistics.base.Arithmetic;
...
public class HyperGeometric extends AbstractDiscreteDistribution {
	...
	private double dl, dr, r1, r2, r4, r5, ll, lr, c_pm, f1, f2, f4, f5, p1,
			p2, p3, p4, p5, p6;

	// The uniform random number generated shared by all <b>static</b> methods.
	protected static HyperGeometric shared = new HyperGeometric(1, 1, 1,
	...
	private static double fc_lnpk(int k, int N_Mn, int M, int n) {
		return (Arithmetic.logFactorial(k) + Arithmetic.logFactorial(M - k)
				+ Arithmetic.logFactorial(n - k) + Arithmetic.logFactorial(N_Mn
				+ k));
	}
	...
	protected int hprs(int N, int M, int n, RandomEngine randomGenerator) {
		...
		for (;;) {
			...
			if ((U = randomGenerator.raw() * p6) < p2) { // centre left
				...
				// immediate acceptance region R2 = [k2, m) *[0, f2), X = k2,
				// ... m -1
				if ((W = U - p1) < 0.0)
					return (k2 + (int) (U / f2));
				// immediate acceptance region R1 = [k1, k2)*[0, f1), X = k1,
				// ... k2-1
				if ((Y = W / dl) < f1)
					return (k1 + (int) (W / f1));
				...
				if ((W = f2 + f2 - Y) < 1.0) { // quick reject of V
					V = k2 + Dk;
					if (W <= f2 + Dk * (1.0 - f2) / (dl + 1.0)) { // quick
																	// accept of
						return (V); // V = k2 + Dk
					}
				...
				// immediate acceptance region R3 = [m, k4+1)*[0, f4), X = m,
				// ... k4
				if ((W = U - p3) < 0.0)
	...
	protected int nextInt(int N, int M, int n, RandomEngine randomGenerator) {
		 ...
		 * Hypergeometric distribution with parameters * N (number of red and
		 * black balls), M (number * of red balls) and n (number of trials) *
		 * valid for N >= 2, M,n <= N. * REFERENCE : - H. Zechner (1994):
		 * Efficient sampling from * continuous and discrete unimodal
		 * distributions, * Doctoral Dissertation, 156 pp., Technical *
		 * University Graz, Austria. * SUBPROGRAMS: - flogfak(k) ... log(k!)
		 * with long integer k * - drand(seed) ... (0,1)-Uniform generator with
		 * * unsigned long integer *seed. * - hmdu(seed,N,M,n) ...
		 * Hypergeometric generator * for l<10 * - hprs(seed,N,M,n) ...
		 * Hypergeometric generator * for l>=10 with unsigned long integer
		 * *seed, * long integer N , M , n. * *
		 ******************************************************************/
		...
		if ((n * M / N) < 10) {
			K = (n_le_Nhalf <= M_le_Nhalf) ? hmdu(N, M_le_Nhalf, n_le_Nhalf,
					randomGenerator) : hmdu(N, n_le_Nhalf, M_le_Nhalf,
					randomGenerator);
		} else {
			K = (n_le_Nhalf <= M_le_Nhalf) ? hprs(N, M_le_Nhalf, n_le_Nhalf,
					randomGenerator) : hprs(N, n_le_Nhalf, M_le_Nhalf,
					randomGenerator);
		}
	...
	public double pdf(int k) {
		return Arithmetic.binomial(my_s, k)
				* Arithmetic.binomial(my_N - my_s, my_n - k)
