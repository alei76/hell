 ...
 */
	protected double b1prs(double p, double q, RandomEngine randomGenerator) {
		...
		for (;;) {
			...
			if (Math.log(W) <= a * Math.log(X / m) + b
					* Math.log((1.0 - X) / (1.0 - m)))
				return (X);
		}
	}
	...
	private static double f(double x, double a, double b, double m) {
		return Math.exp(a * Math.log(x / m) + b
				* Math.log((1.0 - x) / (1.0 - m)));
	}
	...
	public double nextDouble(double alpha, double beta) {
		 ...
		 ******************************************************************
		 * For parameters a < 1 , b < 1 and a < 1 < b or b < 1 < a * the
		 * stratified rejection methods b00 and b01 of Sakasegawa are * used.
		 * Both procedures employ suitable two-part power functions * from which
	...
	public double pdf(double x) {
		if (x < 0 || x > 1)
			return 0.0;
		return Math.exp(PDF_CONST) * Math.pow(x, alpha - 1)
				* Math.pow(1 - x, beta - 1);
	}
	...
	public void setState(double alpha, double beta) {
		this.alpha = alpha;
		this.beta = beta;
		this.PDF_CONST = Fun.logGamma(alpha + beta) - Fun.logGamma(alpha)
				- Fun.logGamma(beta);
	}
