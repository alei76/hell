...
import ps.hell.ml.statistics.base.value.stat.Probability;
...
public class Poisson extends AbstractDiscreteDistribution {
	...
	protected static final double MEAN_MAX = Integer.MAX_VALUE; // for all means
																// larger than
																// that, we
																// don't try to
																// compute a
																...
																// but return
																// the mean.
	protected static final double SWITCH_MEAN = 10.0; // switch from method A to
														// method B
	...
	public int nextInt(double theMean) {
		 ...
		 ******************************************************************
		 * * For parameter my < 10 Tabulated Inversion is applied. * For my >=
		 * 10 Patchwork Rejection is employed: * The area below the histogram
		 * function f(x) is rearranged in * its body by certain point
		 * reflections. Within a large center * interval variates are sampled
		 * efficiently by rejection from * uniform hats. Rectangular immediate
		...
		if (my < SWITCH_MEAN) { // CASE B: Inversion- start new table and
								// calculate p0
			if (my != my_old) {
				my_old = my;
				llll = 0;
			...
			if (my != my_last) { // set-up
				...
				// approximate deviation of reflection points k2, k4 from my -
				// 1/2
				...
				// Poisson constants, necessary for computing function values
				// f(k)
				l_my = Math.log(my);
			...
			for (;;) {
				...
				if ((U = gen.raw() * p6) < p2) { // centre left
					...
					// immediate acceptance region R2 = [k2, m) *[0, f2), X =
					// k2, ... m -1
					if ((V = U - p1) < 0.0)
						return (k2 + (int) (U / f2));
					// immediate acceptance region R1 = [k1, k2)*[0, f1), X =
					// k1, ... k2-1
					if ((W = V / dl) < f1)
						return (k1 + (int) (V / f1));

					// computation of candidate X < k2, and its counterpart Y >
					// k2
					// either squeeze-acceptance of X or acceptance-rejection of
					// Y
					Dk = (int) (dl * gen.raw()) + 1;
					...
					if ((V = f2 + f2 - W) < 1.0) { // quick reject of Y
						Y = k2 + Dk;
						if (V <= f2 + Dk * (1.0 - f2) / (dl + 1.0)) {// quick
																		// accept
																		// of
							return (Y); // Y = k2 + Dk
						}
					...
					// immediate acceptance region R3 = [m, k4+1)*[0, f4), X =
					// m, ... k4
					if ((V = U - p3) < 0.0)
					...
					// computation of candidate X > k4, and its counterpart Y <
					// k4
					// either squeeze-acceptance of X or acceptance-rejection of
					// Y
					Dk = (int) (dr * gen.raw()) + 1;
				...
				// acceptance-rejection test of candidate X from the original
				// area
	...
	public double pdf(int k) {
		return Math.exp(k * Math.log(this.mean) - Arithmetic.logFactorial(k)
				- this.mean);

		// Overflow sensitive:
		// return (Math.pow(mean,k) / cephes.Arithmetic.factorial(k)) *
		// Math.exp(-this.mean);
	}
