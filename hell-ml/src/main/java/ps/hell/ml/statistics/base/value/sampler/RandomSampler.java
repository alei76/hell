...
*/
package ps.hell.ml.statistics.base.value.sampler;

import ps.hell.ml.statistics.base.value.AbstractDistribution;
import ps.hell.ml.statistics.base.value.engine.RandomEngine;
import ps.hell.ml.statistics.base.value.engine.Timer;
...
public RandomSampler(long n, long N, long low, RandomEngine randomGenerator) {
	...
	if (randomGenerator==null) randomGenerator = AbstractDistribution.makeDefaultGenerator();
...
@Override
public Object clone() {
	RandomSampler copy=null;
	try {
		copy = (RandomSampler) super.clone();
...
/**
 * 计算下一个计数排序随机集的随机数字指定实例建设和填充成值,从指数fromIndex。
...
public static void test(long n, long N, long low, int chunkSize, int times) {
	...
	for (long t=times; --t >=0;) {
		RandomSampler sampler = new RandomSampler(n,N,low,AbstractDistribution.makeDefaultGenerator());
