...
public class HyperGeometricDistribution extends DiscreteDistribution {
    ...
    public double logp(int k) {
        if (k < Math.max(0, m + n - N) || k > Math.min(m, n)) {
            ...
            return MathBase.logChoose(m, k) + MathBase.logChoose(N - m, n - k) - MathBase.logChoose(N, n);
        }
    }
    ...
    class Patchwork extends RandomNumberGenerator {
        ...
        private double lnpk(int k, int L, int m, int n) {
            return MathBase.logFactorial(k) + MathBase.logFactorial(m - k) + MathBase.logFactorial(n - k) + MathBase.logFactorial(L + k);
        }
    }
    ...
    class Inversion extends RandomNumberGenerator {
        ...
        Inversion(int N, int m, int n) {
            ...
            fm = Math.exp(MathBase.logFactorial(N - m) - MathBase.logFactorial(L + mode) - MathBase.logFactorial(n - mode) + MathBase.logFactorial(m) - MathBase.logFactorial(m - mode) - MathBase.logFactorial(mode) - MathBase.logFactorial(N) + MathBase.logFactorial(N - n) + MathBase.logFactorial(n));
