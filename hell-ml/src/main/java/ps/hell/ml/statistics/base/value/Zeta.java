...
public class Zeta extends AbstractDiscreteDistribution {
	 ...
	 */
	protected long generateZeta(double ro, double pk,
			RandomEngine randomGenerator) {
		/******************************************************************
		 * * Zeta Distribution - Acceptance Rejection * *
		 ******************************************************************
		 * * To sample from the Zeta distribution with parameters ro and pk * it
		 * suffices to sample variates x from the distribution with * density
		 * function f(x)=B*{[x+0.5]+pk}^(-(1+ro)) ( x > .5 ) * and then deliver
		 * k=[x+0.5]. * 1/B=Sum[(j+pk)^-(ro+1)] (j=1,2,...) converges for ro >=
		 * .5 . * It is not necessary to compute B, because variates x are *
		 * generated by acceptance rejection using density function *
		 * g(x)=ro*(c+0.5)^ro*(c+x)^-(ro+1). * * * Integer overflow is possible,
		 * when ro is small (ro <= .5) and * pk large. In this case a new sample
		 * is generated. If ro and pk * satisfy the inequality ro > .14 +
		 * pk*1.85e-8 + .02*ln(pk) * the percentage of overflow is less than 1%,
		 * so that the * result is reliable. * NOTE: The comment above is likely
		 * to be nomore valid since * the C-version operated on 32-bit integers,
		 * while this Java * version operates on 64-bit integers. However, the
		 * following is * still valid: * * * * If either ro > 100 or k > 10000
		 * numerical problems in * computing the theoretical moments arise,
		 * therefore ro<=100 and * k<=10000 are recommended. * *
		 ******************************************************************
		 * * FUNCTION: - zeta samples a random number from the * Zeta
		 * distribution with parameters ro > 0 and * pk >= 0. * REFERENCE: - J.
		 * Dagpunar (1988): Principles of Random * Variate Generation, Clarendon
		 * Press, Oxford. * *
		 ******************************************************************/
		double u, v, e, x;
		long k;
