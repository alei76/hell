...
public class Binomial extends AbstractDiscreteDistribution {
	...
	protected static Binomial shared = new Binomial(1, 0.5,
	 ...
	 * - k. * *
	 ******************************************************************
	 * * FUNCTION: - samples a random number from the binomial * distribution
	 * with parameters n and p and is * valid for n*min(p,1-p) > 0. * REFERENCE:
	 * - V. Kachitvichyanukul, B.W. Schmeiser (1988): * Binomial random variate
				...
				if (V <= T + E) {
					if (n != n_prev || par != p_prev) {
						...
						ch = xm * Math.log((m + 1.0) / (pq * nm))
								+ Arithmetic.stirlingCorrection(m + 1)
								+ Arithmetic.stirlingCorrection(nm);
					}
					nK = n - K + 1;
					...
					if (V <= ch + (n + 1.0)
							* Math.log((double) nm / (double) nK) + (K + 0.5)
							* Math.log(nK * pq / (K + 1.0))
							- Arithmetic.stirlingCorrection(K + 1)
	...
	public double pdf(int k) {
		if (k < 0)
			throw new IllegalArgumentException();
		int r = this.n - k;
		return Math.exp(this.log_n - Arithmetic.logFactorial(k)
				- Arithmetic.logFactorial(r) + this.log_p * k + this.log_q * r);
	}
