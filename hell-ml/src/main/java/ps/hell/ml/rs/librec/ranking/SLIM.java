...
package ps.hell.ml.rs.librec.ranking;
...
import ps.hell.ml.rs.librec.data.VectorEntry;
		...
		if (knn > 0) {
			...
			for (int j = 0; j < numItems; j++) {
				...
				if (knn > 0 && knn < nns.size()) {
					List<Entry<Integer, Double>> sorted = Lists.sortMap(nns, true);
					List<Entry<Integer, Double>> subset = sorted.subList(0, knn);
					nns.clear();
					for (Entry<Integer, Double> kv : subset)
