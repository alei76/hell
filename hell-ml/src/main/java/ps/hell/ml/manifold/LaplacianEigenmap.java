...
/**
 * 拉普拉斯算子Eigenmap。使用最近的拉普拉斯算子的概念
*邻居邻接图,拉普拉斯算子Eigenmap计算低维
*代表数据集的最优保存当地的社区
*在某种意义上的信息。表示生成的地图
*算法可以被视为一个离散近似连续映射
*自然来自流形的几何
...
public class LaplacianEigenmap {
    ...
    public LaplacianEigenmap(double[][] data, int d, int k, double t) {
        ...
        if (t <= 0) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                Collection<Edge> edges = graph.getEdges(i);
                    ...
                    double w = Math.exp(gamma * MathBase.sqr(edge.weight));
                    W.set(i, j, w);
                    D[i] += w;
                }
        ...
        // Largest eigenvalue.
        double lambda = -MathBase.eigen(L, v, 1E-6);
        for (int i = 0; i < n; i++) {
            L.set(i, i, lambda - 1.0);
        }

        EigenValueDecomposition2 eigen = EigenValueDecomposition2.decompose(L, d + 1);
