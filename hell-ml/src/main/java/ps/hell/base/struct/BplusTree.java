	...
	public class Node {
		...
		public Object get(Comparable key) {
			...
			if (isLeaf) {
				...
				} else if (key.compareTo(entries.get(entries.size() - 1)
						.getKey()) >= 0) {
			...
			if (isLeaf) {
				...
				if (key.compareTo(entries.get(0).getKey()) <= 0) {
					...
					// 如果key大于节点最右边的key，沿最后一个子节点继续搜索
						...
						.getKey()) >= 0) {
					children.get(children.size() - 1).insertOrUpdate(key, obj,
							tree);
					// 否则沿比key大的前一个子节点继续搜索
				} else {
					for (int i = 0; i < entries.size(); i++) {
		...
		protected void updateInsert(BplusTree tree) {
			...
			if (children.size() > tree.getOrder()) {
				...
				for (int i = 0; i < leftSize; i++) {
					...
					left.getEntries().add(
							new SimpleEntry(children.get(i).getEntries().get(0)
									.getKey(), null));
					children.get(i).setParent(left);
				}
				for (int i = 0; i < rightSize; i++) {
					right.getChildren().add(children.get(leftSize + i));
					right.getEntries().add(
							new SimpleEntry(children.get(leftSize + i)
									.getEntries().get(0).getKey(), null));
		...
		protected void validate(Node node, BplusTree tree) {
			...
			if (node.getEntries().size() == node.getChildren().size()) {
				for (int i = 0; i < node.getEntries().size(); i++) {
					Comparable key = node.getChildren().get(i).getEntries()
							.get(0).getKey();
					...
					if (previous != null
						...
						if (previous != null
							...
							// 同后面节点合并
								...
								&& (next.getChildren().size() <= tree
										.getOrder() / 2 || next.getChildren()
										.size() <= 2)) {
				...
				if (isRoot) {
					...
					if (entries.size() > tree.getOrder() / 2
						...
						if (previous != null
							...
							Entry<Comparable, Object> entry = next.getEntries()
									.get(0);
