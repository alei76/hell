 ...
 * @see java.util.Arrays
 */
public static int binarySearchFromTo(int from, int to) {
	final int dummy = 0;
	while (from <= to) {
		int mid = (from + to) / 2;
		int comparison = Integer.compare(dummy,mid);
...
private static void mergeSort1(byte src[], byte dest[], int low, int high) {
	...
	if (length < SMALL) {
	    for (int i=low; i<high; i++)
			for (int j=i; j>low && Byte.compare(dest[j-1], dest[j])>0; j--)
			    swap(dest, j, j-1);
	    return;
	}
	...
	if (Byte.compare(src[mid-1], src[mid]) <= 0) {
	...
	for(int i = low, p = low, q = mid; i < high; i++) {
		if (q>=high || p<mid && Byte.compare(src[p], src[q]) <= 0)
			dest[i] = src[p++];
		else
			dest[i] = src[q++];
	}
...
private static void mergeSort1(char src[], char dest[], int low, int high) {
	...
	if (length < SMALL) {
	    for (int i=low; i<high; i++)
			for (int j=i; j>low && Character.compare(dest[j-1], dest[j])>0; j--)
			    swap(dest, j, j-1);
	    return;
	}
	...
	// If list is already sorted, just copy from src to dest.  This is an
	// optimization that results in faster sorts for nearly ordered lists.
	if (Character.compare(src[mid-1], src[mid]) <= 0) {
	   System.arraycopy(src, low, dest, low, length);
	   return;
	}
	...
	// Merge sorted halves (now in src) into dest
	for(int i = low, p = low, q = mid; i < high; i++) {
		if (q>=high || p<mid && Character.compare(src[p], src[q]) <= 0)
			dest[i] = src[p++];
		else
			dest[i] = src[q++];
	}
...
private static void mergeSort1(double src[], double dest[], int low, int high) {
	...
	if (length < SMALL) {
	    for (int i=low; i<high; i++)
			for (int j=i; j>low && Double.compare(dest[j-1], dest[j])>0; j--)
			    swap(dest, j, j-1);
	    return;
	}
...
private static void mergeSort1(float src[], float dest[], int low, int high) {
	...
	if (length < SMALL) {
	    for (int i=low; i<high; i++)
			for (int j=i; j>low && Float.compare(dest[j-1], dest[j])>0; j--)
			    swap(dest, j, j-1);
	    return;
	}
	...
	// If list is already sorted, just copy from src to dest.  This is an
	// optimization that results in faster sorts for nearly ordered lists.
	if (Float.compare(src[mid-1], src[mid]) <= 0) {
	   System.arraycopy(src, low, dest, low, length);
	   return;
	}
	...
	// Merge sorted halves (now in src) into dest
	for(int i = low, p = low, q = mid; i < high; i++) {
		if (q>=high || p<mid && Float.compare(src[p], src[q]) <= 0)
			dest[i] = src[p++];
		else
			dest[i] = src[q++];
	}
...
private static void mergeSort1(long src[], long dest[], int low, int high) {
	...
	if (length < SMALL) {
	    for (int i=low; i<high; i++)
			for (int j=i; j>low && Long.compare(dest[j-1], dest[j])>0; j--)
			    swap(dest, j, j-1);
	    return;
	}
	...
	// If list is already sorted, just copy from src to dest.  This is an
	// optimization that results in faster sorts for nearly ordered lists.
	if (Long.compare(src[mid-1], src[mid]) <= 0) {
	   System.arraycopy(src, low, dest, low, length);
	   return;
	}
	...
	// Merge sorted halves (now in src) into dest
	for(int i = low, p = low, q = mid; i < high; i++) {
		if (q>=high || p<mid && Long.compare(src[p], src[q]) <= 0)
			dest[i] = src[p++];
		else
			dest[i] = src[q++];
	}
...
private static void mergeSort1(short src[], short dest[], int low, int high) {
	...
	if (length < SMALL) {
	    for (int i=low; i<high; i++)
			for (int j=i; j>low && Short.compare(dest[j-1], dest[j])>0; j--)
			    swap(dest, j, j-1);
	    return;
	}
	...
	// If list is already sorted, just copy from src to dest.  This is an
	// optimization that results in faster sorts for nearly ordered lists.
	if (Short.compare(src[mid-1], src[mid]) <= 0) {
	   System.arraycopy(src, low, dest, low, length);
	   return;
	}
	...
	// Merge sorted halves (now in src) into dest
	for(int i = low, p = low, q = mid; i < high; i++) {
		if (q>=high || p<mid && Short.compare(src[p], src[q]) <= 0)
			dest[i] = src[p++];
		else
			dest[i] = src[q++];
	}
...
if (numNegZeros != 0) {
	int j = binarySearchFromTo(a,0.0d, fromIndex, n-1); // posn of ANY zero
	do {
		j--;
	} while (j>=0 && a[j]==0.0d);
...
// Postprocessing phase: change 0.0's to -0.0's as required
if (numNegZeros != 0) {
	int j =binarySearchFromTo(a,0.0f, fromIndex, n-1); // posn of ANY zero
	do {
		j--;
	} while (j>=0 && a[j]==0.0f);
 ...
 * @see Comparator
 */
public static void quickSort(byte[] a, int fromIndex, int toIndex) {
	rangeCheck(a.length, fromIndex, toIndex);
 ...
 * @see Comparator
 */
public static void quickSort(char[] a, int fromIndex, int toIndex) {
	rangeCheck(a.length, fromIndex, toIndex);
	quickSort1(a, fromIndex, toIndex-fromIndex);
}
/**
 ...
 * @see Comparator
 */
public static void quickSort(double[] a, int fromIndex, int toIndex) {
	rangeCheck(a.length, fromIndex, toIndex);
	quickSort1(a, fromIndex, toIndex-fromIndex);
}
/**
 ...
 * @see Comparator
 */
public static void quickSort(float[] a, int fromIndex, int toIndex) {
	rangeCheck(a.length, fromIndex, toIndex);
	quickSort1(a, fromIndex, toIndex-fromIndex);
}
/**
 ...
 * @see Comparator
 */
public static void quickSort(short[] a, int fromIndex, int toIndex) {
	rangeCheck(a.length, fromIndex, toIndex);
	quickSort1(a, fromIndex, toIndex-fromIndex);
}
/**
 ...
 */
private static void quickSort1(byte x[], int off, int len) {
	// Insertion sort on smallest arrays
	if (len < SMALL) {
		for (int i=off; i<len+off; i++)
		for (int j=i; j>off && Byte.compare(x[j-1],x[j])>0; j--)
		    swap(x, j, j-1);
		return;
	}
 ...
 * Sorts the specified sub-array of chars into ascending order.
 */
private static void quickSort1(char x[], int off, int len) {
	// Insertion sort on smallest arrays
	if (len < SMALL) {
		for (int i=off; i<len+off; i++)
		for (int j=i; j>off && Character.compare(x[j-1],x[j])>0; j--)
		    swap(x, j, j-1);
		return;
	}
 ...
 * Sorts the specified sub-array of chars into ascending order.
 */
private static void quickSort1(double x[], int off, int len) {
	// Insertion sort on smallest arrays
	if (len < SMALL) {
		for (int i=off; i<len+off; i++)
		for (int j=i; j>off && Double.compare(x[j-1],x[j])>0; j--)
		    swap(x, j, j-1);
		return;
	}
 ...
 * Sorts the specified sub-array of chars into ascending order.
 */
private static void quickSort1(float x[], int off, int len) {
	// Insertion sort on smallest arrays
	if (len < SMALL) {
		for (int i=off; i<len+off; i++)
		for (int j=i; j>off && Float.compare(x[j-1],x[j])>0; j--)
		    swap(x, j, j-1);
		return;
	}
 ...
 * Sorts the specified sub-array of chars into ascending order.
 */
private static void quickSort1(int x[], int off, int len) {
	// Insertion sort on smallest arrays
	if (len < SMALL) {
		for (int i=off; i<len+off; i++)
		for (int j=i; j>off && Integer.compare(x[j-1],x[j])>0; j--)
		    swap(x, j, j-1);
		return;
	}
...
private static void quickSort1(long x[], int off, int len) {
	...
	if (len < SMALL) {
		for (int i=off; i<len+off; i++)
		for (int j=i; j>off && Long.compare(x[j-1],x[j])>0; j--)
		    swap(x, j, j-1);
		return;
	}
 ...
 * Sorts the specified sub-array of chars into ascending order.
 */
private static void quickSort1(short x[], int off, int len) {
	// Insertion sort on smallest arrays
	if (len < SMALL) {
		for (int i=off; i<len+off; i++)
		for (int j=i; j>off && Short.compare(x[j-1],x[j])>0; j--)
		    swap(x, j, j-1);
		return;
	}
