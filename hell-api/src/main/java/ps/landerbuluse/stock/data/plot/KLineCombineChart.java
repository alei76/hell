...
public class KLineCombineChart {
    ...
    double high3Value = Double.MIN_VALUE;//设置成交量的最大值
    double min3Value = Double.MAX_VALUE;//设置成交量的最低值
    OHLCSeries series = new OHLCSeries("");//高开低收数据序列，股票K线图的四个数据，依次是开，高，低，收
    TimeSeries series2=new TimeSeries("");//对应时间成交量数据
    TimeSeries series3=new TimeSeries("");//对应原始时间 rate
    TimeSeries series4=new TimeSeries("");//对应 模型时间 rate
    final OHLCSeriesCollection seriesCollection = new OHLCSeriesCollection();//保留K线数据的数据集，必须申明为final，后面要在匿名内部类里面用到
    TimeSeriesCollection timeSeriesCollection=new TimeSeriesCollection();//保留成交量数据的集合
    TimeSeriesCollection timeSeriesRateCollection=new TimeSeriesCollection();//保留成交量数据的集合
    StockData stockData = null;
    public KLineCombineChart(){

    }

    public void addData(StockData stockData){
        this.stockData = stockData;
        ArrayList<StockBean> beans = stockData.getData();
        float source =0f;
        for(StockBean bean:beans){
//            System.out.println(bean.date);
            Day day = getDay(bean.date);
            series.add(day, bean.openPrice, bean.getHightPrice(),bean.getLowerPrice(),bean.getClosePrice());
            series2.add(day, bean.urnover/100);
            if(source == 0f) {
                series3.add(day,1);
                source = bean.openPrice;
            }else{
                series3.add(day,bean.closePrice/source);
            }
//            series3.add(day,bean.openPrice);
//            series4.add(day,bean.closePrice);
        }
        seriesCollection.addSeries(series);
        timeSeriesCollection.addSeries(series2);
        timeSeriesRateCollection.addSeries(series3);
        timeSeriesRateCollection.addSeries(series4);

    }

    public Day getDay(String date){
        return new Day(Integer.parseInt(date.substring(8)), Integer.parseInt(date.substring(5,7)),
                Integer.parseInt( date.substring(0,4)));
    }

    public void addRateData(float sourcePrice,ArrayList<AccountLogger> accountLogger){
        for(int i=0;i<accountLogger.size();i++) {
            AccountLogger bean = accountLogger.get(i);
            Day day = getDay(bean.getDate());
            series4.add(day,bean.getAccountAll()/sourcePrice);
        }
    }
    ...
    public void plot(){
        ...
        int seriesCount3 = timeSeriesRateCollection.getSeriesCount();//一共有多少个序列，目前为一个
        for (int i = 0; i < seriesCount2; i++) {
            int itemCount = timeSeriesRateCollection.getItemCount(i);//每一个序列有多少个数据项
            for (int j = 0; j < itemCount; j++) {
                if (high3Value < timeSeriesRateCollection.getYValue(i,j)) {//取第i个序列中的第j个数据项的值
                    high3Value = timeSeriesRateCollection.getYValue(i,j);
                }
                if (min3Value > timeSeriesRateCollection.getYValue(i, j)) {//取第i个序列中的第j个数据项的值
                    min3Value = timeSeriesRateCollection.getYValue(i, j);
                }
            }
        }
        ...
        try{
            x1Axis.setRange(dateFormat.parse(stockData.start),dateFormat.parse(stockData.end));//设置时间范围，注意时间的最大值要比已有的时间最大值要多一天
        }catch(Exception e){
            e.printStackTrace();
        }
        ...
        x1Axis.setTickUnit(new DateTickUnit(DateTickUnit.MONTH, 1));//设置时间刻度的间隔，一般以周为单位
        ...
        //y1Axis.setTickUnit(new NumberTickUnit((highValue * 1.1 - minValue * 0.9) / 100));//设置刻度显示的密度
        y1Axis.setTickUnit(new NumberTickUnit((highValue * 1.1 - minValue * 0.9) / 4));
...
//        y2Axis.setTickUnit(new NumberTickUnit(());
        ...
        XYLineAndShapeRenderer renderer = new XYLineAndShapeRenderer();//(XYLineAndShapeRenderer) plot.getRenderer();
        ...
        XYPlot plot3=new XYPlot(timeSeriesRateCollection,null,y3Axis,renderer);//建立第二个画图区域对象，主要此时的x轴设为了null值，因为要与第一个画图区域对象共享x轴
        ...
        JFreeChart chart = new JFreeChart(stockData.getStockName(), JFreeChart.DEFAULT_TITLE_FONT, combineddomainxyplot, false);
        ChartFrame frame = new ChartFrame(stockData.getStockName(), chart);
        frame.pack();
